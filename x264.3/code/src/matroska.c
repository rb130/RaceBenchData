...
int mk_writeHeader(mk_Writer *w, const char *writingApp, const char *codecID, const void *codecPrivate, unsigned codecPrivateSize, int64_t default_frame_duration, int64_t timescale, unsigned width, unsigned height, unsigned d_width, unsigned d_height) {
    ...
    if ((c = mk_createContext(w, w->root, 0x1a45dfa3)) == NULL) {
        return -1;
    }
    CHECK(mk_writeUInt(c, 0x4286, 1));
    CHECK(mk_writeUInt(c, 0x42f7, 1));
    CHECK(mk_writeUInt(c, 0x42f2, 4));
    CHECK(mk_writeUInt(c, 0x42f3, 8));
    CHECK(mk_writeStr(c, 0x4282, "matroska"));
    CHECK(mk_writeUInt(c, 0x4287, 1));
    CHECK(mk_writeUInt(c, 0x4285, 1));
    CHECK(mk_closeContext(c, 0));

    if ((c = mk_createContext(w, w->root, 0x18538067)) == NULL) {
        return -1;
    }
    CHECK(mk_flushContextID(c));
    CHECK(mk_closeContext(c, 0));

    if ((c = mk_createContext(w, w->root, 0x1549a966)) == NULL) {
        return -1;
    }
    ...
    if ((ti = mk_createContext(w, c, 0xae)) == NULL) {
        return -1;
    }
    CHECK(mk_writeUInt(ti, 0xd7, 1));
    CHECK(mk_writeUInt(ti, 0x73c5, 1));
    CHECK(mk_writeUInt(ti, 0x83, 1));
    CHECK(mk_writeUInt(ti, 0x9c, 0));
    CHECK(mk_writeStr(ti, 0x86, codecID));
    if (codecPrivateSize) {
        CHECK(mk_writeBin(ti, 0x63a2, codecPrivate, codecPrivateSize));
    }
    if (default_frame_duration) {
        CHECK(mk_writeUInt(ti, 0x23e383, default_frame_duration));
    }
...
static int mk_flushFrame(mk_Writer *w) {
    ...
    if (w->cluster == NULL) {
        ...
        w->cluster = mk_createContext(w, w->root, 0x1f43b675);
        if (w->cluster == NULL) {
            return -1;
        }

        CHECK(mk_writeUInt(w->cluster, 0xe7, w->cluster_tc_scaled));
    ...
    CHECK(mk_writeID(w->cluster, 0xa0));
    CHECK(mk_writeSize(w->cluster, bgsize));
    CHECK(mk_writeID(w->cluster, 0xa1));
    CHECK(mk_writeSize(w->cluster, fsize + 4));
    CHECK(mk_writeSize(w->cluster, 1));
    ...
    if (!w->keyframe) {
        CHECK(mk_writeSInt(w->cluster, 0xfb, ref));
    }
