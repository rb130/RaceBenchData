...
int cass_table_import_data(cass_table_t *table, char *fname) {
    ...
    for (;;) {
        ...
        if (table->cfg->vecset_type == CASS_VECSET_SINGLE && l != 1) {
            goto err;
        }
        ...
        for (j = 0; j < l; j++) {
            ...
            eat_comment(fin);
            if (fscanf(fin, "%f", &vec->weight) != 1) {
                goto err;
            }
            switch (table->cfg->vec_type) {
            case CASS_VEC_INT:
                ret = _import_vec_int(vec->u.int_data, dim, fin);
                break;
            case CASS_VEC_FLOAT:
                ret = _import_vec_float(vec->u.float_data, dim, fin);
                break;
            case CASS_VEC_BIT:
                ret = _import_vec_bit(vec->u.bit_data, dim, fin);
                break;
            default:
                assert(0);
            }
...
int cass_table_export_data(cass_table_t *table, char *fname) {
    ...
    assert(table->opr->type & CASS_DATA || table->opr->type & CASS_VEC_SKETCH);
    ...
    if (table->cfg->vecset_type == CASS_VECSET_NONE) {
        k = 0;
    } else if (table->cfg->vecset_type == CASS_VECSET_SINGLE) {
        k = 1;
    } else if (table->cfg->vecset_type == CASS_VECSET_SET) {
        k = 2;
    } else {
        assert(0);
    }
    ...
    if ((ds->flags & CASS_DATASET_BOTH) == CASS_DATASET_BOTH) {
        ...
        for (i = 0; i < ds->num_vecset; i++) {
            ...
            for (j = 0; j < ds->vecset[i].num_regions; j++) {
                ...
                switch (table->cfg->vec_type) {
                case CASS_VEC_INT:
                    _export_vec_int(vec->u.int_data, dim, fout);
                    break;
                case CASS_VEC_FLOAT:
                    _export_vec_float(vec->u.float_data, dim, fout);
                    break;
                case CASS_VEC_BIT:
                    _export_vec_bit(vec->u.bit_data, dim, fout);
                    break;
                default:
                    assert(0);
                }
