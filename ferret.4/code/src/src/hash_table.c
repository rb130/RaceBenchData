...
void bucketT_insert(bucketT *bucket, int val) {
...
void bucketT_free(bucketT *bucket) {
#ifdef NOTDEF
    nodeT *p, *q;
...
int bucketT_dump(bucketT *bucket, CASS_FILE *out) {
    int ret;
...
bucketT *bucketT_load(int idx, CASS_FILE *in) {
    int i, ret, cnt, val;
...
int hashTable_free(hashTable_t *hash_table) {
    int i;
...
int hashTable_dump(hashTable_t *hash_table, CASS_FILE *out) {
    ...
    ret = cass_write_uint32(hash_table->rndVec, hash_table->key_size, out);
...
hashTable_t *hashTable_load(CASS_FILE *in) {
    ...
    hash_table->buckets = (bucketT **)calloc(hash_table->size, sizeof(bucketT *));
...
int hashTable_hash(hashTable_t *table, hashT key) {
    int i, k, sum = 0;
