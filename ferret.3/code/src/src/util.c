...
int mkpath(char *buf, const char *base, const char *table, const char *ext) {
    if (snprintf(buf, BUFSIZ, "%s/%s.%s", base, table, ext) >= BUFSIZ) {
        assert(0);
    }
    return 0;
}
...
int cass_write_pchar(const char *buf, CASS_FILE *out) {
    ...
    if (cass_write_size(&len, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_char(buf, len, out) != len) {
        return CASS_ERR_IO;
    }
    return 0;
}
...
int param_get_float_array(const char *param, const char *key, cass_size_t *_n, float *f) {
    cass_size_t n = 0;
...
cass_size_t cass_vec_dim2size(cass_vec_type_t type, cass_size_t dim) {
    ...
    case CASS_VEC_FLOAT:
        ...
        break;
    case CASS_VEC_BIT:
        size += (dim + CHUNK_BIT - 1) / CHUNK_BIT * sizeof(chunk_t);
        break;
    default:
        assert(0);
    }
...
int cass_result_free(cass_result_t *result) {
    if (result->flags & CASS_RESULT_LIST) {
        ...
        debug("cass_result_free: unsupported formate.\n");
    }
