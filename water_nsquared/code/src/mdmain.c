...
double MDMAIN(NFSV, NFRST, NSTEP, NRST, NPRINT, NSAVE, LKT, NORD1, ProcID)
    ...
    int NFSV,
    NFRST, NSTEP, NRST, NPRINT, NSAVE, LKT, NORD1;
unsigned ProcID;

{
    ...
    double TTMV = 0.0, TKIN = 0.0, TVIR = 0.0; racebench_bug12(1); racebench_bug29(1); racebench_bug59(0);

    INTRAF(&gl->VIR, ProcID); racebench_bug11(0); racebench_bug25(0); racebench_bug88(1);

    {
        unsigned long Error, Cycle;
        int Cancel, Temp;

        Error = pthread_mutex_lock(&(gl->start).mutex); racebench_bug88(0);
        if (Error != 0) {
            printf("Error while trying to get lock in barrier.\n");
            exit(-1);
        }

        Cycle = (gl->start).cycle; racebench_bug6(1); racebench_bug69(1);
