...
void SweepBVHBuilder::recursiveBuild(const int begin, const int end, const int nodeIndex, const AABB &bounds, const sse_f *const centroid, int *const item, AABB *const bvh, int &numNodes, const int depth) {
    AABB leftBounds, rightBounds;
    const unsigned int items = end - begin + 1;
    assert(items > 0);

    if ((items <= MIN_ITEMS) || (depth > MAX_DEPTH) || bounds.m_min == bounds.m_max) {
    createLeaf:
...
void SweepBVHBuilder::my_build(const AABB *const aabb, int *const item, AABB *const bvh, const int numAABBs) {
...
void SweepBVHBuilder::checkTree(AABB *const bvh, const int *const item, const unsigned int index) {
...
unsigned int SweepBVHBuilder::adjustBounds(AABB *const bvh, const AABB *const aabb, const int *const item, const unsigned int index, const unsigned int begin) {
    AABB &entry = bvh[index];
