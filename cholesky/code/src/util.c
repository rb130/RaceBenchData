...
SMatrix ReadSparse(name, probName) char *name, *probName;
{
    FILE *fp;
    int n, m, i, j;
    int n_rows, tmp;
...
ParseIntFormat(buf, num, size) char *buf;
int *num, *size;
{
    char *tmp;
...
ReadVector(fp, n, where, perline, persize) FILE *fp;
int *where;
{
    int i, j, item;
    char tmp, buf[100];
...
SMatrix LowerToFull(L) SMatrix L;
{
    SMatrix M;
    int *link, *first;
    int i, j, nextj, ind = 0;
