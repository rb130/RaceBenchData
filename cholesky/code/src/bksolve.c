...
double *TriBSolve(LB, b, PERM, INVP) BMatrix LB;
double *b;
int *PERM, *INVP;
{
    ...
    for (j = 0; j < LB.n; j += LB.partition_size[j]) {
        if (LB.domain[j]) {
            ...
            for (j1 = j; j1 < j + LB.partition_size[j]; j1++) {
                y[j1] = bt[PERM[j1]] / LB.entry[bl].block->nz[(j1 - j) + (j1 - j) * LB.entry[bl].block->length];
                for (i1 = j1 - j + 1; i1 < LB.entry[bl].block->length; i1++) {
                    ...
                    bt[PERM[row]] -= LB.entry[bl].block->nz[i1 + (j1 - j) * LB.entry[bl].block->length] * y[j1];
                }
            }
        ...
        if (LB.domain[j]) {
            ...
            for (bl = LB.col[j] + 1; bl < LB.col[j + 1]; bl++) {
                for (i1 = 0; i1 < LB.entry[bl].block->length; i1++) {
                    ...
                    for (j1 = j; j1 < j + LB.partition_size[j]; j1++) {
                        y[j1] -= LB.entry[bl].block->nz[i1 + (j1 - j) * LB.entry[bl].block->length] * xp[row];
                    }
                }
            }
            ...
            for (j1 = j + LB.partition_size[j] - 1; j1 >= j; j1--) {
                ...
                xp[j1] = y[j1] / LB.entry[bl].block->nz[(j1 - j) + (j1 - j) * LB.entry[bl].block->length];
            }
        }
...
double ComputeNorm(x, n) double *x;
{
    double tmp = 0.0;
    int i;
...
double *CreateVector(M) SMatrix M;
{
    int i, j;
    double *b, Value();
