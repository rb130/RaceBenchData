...
Partition(M, parts, T, assigned_ops, domain, domains, proc_domains, distribute) SMatrix M;
int *T, *assigned_ops, *domain, *domains, *proc_domains;
{
    ...
    for (;;) {
        ...
        if (maxm == 0 || (100.0 * (maxm - minm) / (double)maxm < tolerance) || (1.0 * parts * maxm / work_tree[M.n] < 0.05)) {
            break;
        }
...
MarkSubtreeAsDomain(domain, root) int *domain;
{
...
NumberPartition(parts, assigned_ops, distribute) int *assigned_ops;
{
    int i, minm;
...
MaxBucket(assigned_ops, parts) int *assigned_ops;
{
    int i, maxm, ind;
...
MinBucket(assigned_ops, parts) int *assigned_ops;
{
    int i, minm, ind;
