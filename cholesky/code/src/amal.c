...
CountSupers(cols, node) int *node;
{
    int i, supers;
...
Amalgamate2(join, M, T, nz, node, domain, target_size) SMatrix M;
...
ConsiderMerge(join, super, M, nz, node, domain, target_size, traversal_order) SMatrix M;
int *nz, *node, *domain;
{
    int i, parent;
    ...
    for (;;) {
        ...
        PMOD(node[super], nz[super] - node[super], nz[super] - node[super], &dummy, &dummy, &time_before);
        PADD(nz[super] - node[super], nz[super] - node[super], &dummy, &time_before);
        PDIV(node[parent], nz[parent], &dummy, &dummy, &time_before);
        PMOD(node[parent], nz[parent] - node[parent], nz[parent] - node[parent], &dummy, &dummy, &time_before);
        PADD(nz[parent] - node[parent], nz[parent] - node[parent], &dummy, &time_before);

        time_after = 0;
        PDIV(node[super] + node[parent], node[super] + nz[parent], &dummy, &dummy, &time_after);
        PMOD(node[super] + node[parent], nz[parent] - node[parent], nz[parent] - node[parent], &dummy, &dummy, &time_after);
        PADD(nz[parent] - node[parent], nz[parent] - node[parent], &dummy, &time_after);

        simple_diff = (ops_after - ops_before) - 5.0 * 3 * (nz[super] - node[super]) * (nz[super] - node[super] + 1) / 2;
...
JoinTwoSupers2(nz, node, domain, child, parent, target_size) int *nz, *node, *domain;
{
...
ReorderMatrix(M, super, node, counter, PERM) SMatrix M;
...
FixNodeNZAndT(M, PERM, node, nz, T) SMatrix M;
int *PERM, *node, *nz, *T;
{
    int super, j;
    int *tmp;
...
InvertPerm(n, PERM, INVP) int *PERM, *INVP;
{
    int i;
