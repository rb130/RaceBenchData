...
ComputeTargetBlockSize(M, P) SMatrix M;
{
    int max_ht;
    double total_ops;
...
FindMaxHeight(L, root, height, maxm) SMatrix L;
int *maxm;
{
    int i;
...
CreatePermutation(n, node, PERM, permutation_method) int *node, *PERM;
{
    int j, k;
    int swap, tmp;
    extern int *domain;
