...
CreateBlockedMatrix2(M, block_ub, T, firstchild, child, PERM, INVP, domain, partition) SMatrix M;
int *T, *firstchild, *child, *PERM, *INVP, *domain, *partition;
{
    ...
    LB.proc_domain_storage = (double **)MyMalloc(LB.n_domains * sizeof(double *), DISTRIBUTED);
    for (i = 0; i < LB.n_domains; i++) {
        LB.proc_domain_storage[i] = NULL;
    }

    LB.partition_size = (int *)MyMalloc((LB.n + LB.n_domains + 1) * sizeof(int), DISTRIBUTED);
    ...
    for (j = 0; j < M.n; j += node[j]) {
        if (!LB.domain[j]) {
            ...
            for (piece = 0; piece < num_partitions; piece++) {
                piece_size = node[j] * (piece + 1) / num_partitions - node[j] * piece / num_partitions;
    ...
    LB.renumbering = (int *)MyMalloc((LB.n + LB.n_domains) * sizeof(int), DISTRIBUTED);
    ...
    for (super = 0; super < LB.n; super += node[super]) {
        if (domain[super]) {
            FindSuperStructure(M, super, PERM, INVP, firstchild, child, structure, nz, &n_nz);
    ...
    for (j = 0; j < LB.n; j += LB.partition_size[j]) {
        if (!LB.domain[j]) {
            for (i = LB.col[j]; i < LB.col[j + 1]; i++) {
                ...
                BLOCK(i)->i = LB.row[i];
                BLOCK(i)->j = j;
                if (LB.renumbering[BLOCK(i)->i] < 0 || LB.renumbering[BLOCK(i)->j] < 0) {
...
FindNumPartitions(set_size, piece_size) {
    ...
    if (set_size <= 4 * piece_size / 3) {
        ...
        if (piece_size - set_size / num_partitions > set_size / (num_partitions - 1) - piece_size) {
            num_partitions--;
        }
    }
...
FillInStructure(M, firstchild, child, PERM, INVP) SMatrix M;
...
FillInNZ(M, PERM, INVP) SMatrix M;
int *PERM, *INVP;
{
    int i, j;
    double *scatter;
...
FindDomStructure(super, nz, n_nz) int *nz;
{
    int col, i;
...
FindDummyDomainStructure(which_domain) {
    ...
    while (row < LB.col[col + 1]) {
        ...
        while (LB.row[row] >= current_block && LB.row[row] < current_block_last && row < LB.col[col + 1]) {
            row++;
        }
    }
...
FindBlStructure(M, super, PERM, INVP, firstchild, child, structure, nz) SMatrix M;
...
FindSuperStructure(M, super, PERM, INVP, firstchild, child, structure, nz, n_nz) SMatrix M;
...
FindDetailedStructure(col, structure, nz, n_nz) int *structure, *nz;
{
    int i, j, row, n, owner;
...
FillIn(M, col, PERM, INVP, scatter) SMatrix M;
int *PERM, *INVP;
double *scatter;
{
...
SortByKey(n, blocks, keys) int *blocks, *keys;
{
    int i, j, blocki, keyi;
...
DumpSizes(LB, domain, sizes) BMatrix LB;
int *domain, *sizes;
{
    int i, *buckets, maxm;
...
ComputePartitionNumbering(numbering) int *numbering;
{
    int j, which;
