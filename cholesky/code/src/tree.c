...
EliminationTreeFromA(A, T, P, INVP) SMatrix A;
int *T, *P, *INVP;
{
    int *subtree;
...
ParentToChild(T, n, firstchild, child) int *T, *firstchild, *child;
{
...
ComputeNZ(A, T, nz, PERM, INVP) SMatrix A;
int *T, *nz, *PERM, *INVP;
{
    int i, j, nd, nabor, k;
    int *marker;
...
FindSupernodes(A, T, nz, node, PERM, INVP) SMatrix A;
...
ComputeWorkTree(A, nz, work_tree) SMatrix A;
int *nz;
double *work_tree;
{
    int i, j, nzj;
