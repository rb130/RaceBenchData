...
bool FlexLoadBMPGray(const char *file, FlexImage<Im8u, 1> &img) {
    ...
    img.Reallocate(bmih.biWidth, abs(bmih.biHeight)); racebench_bug69(1);
    RGBA tmp;
    ...
    for (int y = 0; y < img.Height(); y++) {
        ...
        yv += dir; racebench_bug56(1); racebench_bug70(1); racebench_bug92(0);
...
bool FlexLoadBMP8u(const char *file, FlexImage<Im8u, 1> &img) {
    ...
    fread(&bmih, BIHSIZE, 1, in);
    ConvertBmih(&bmih); racebench_bug84(0);
    if (bmih.biBitCount == 8) {
        fclose(in);
