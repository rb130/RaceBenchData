...
void BodyGeometry::ComputeGeometry(BodyPose &angles, BodyParameters &params) {
    ...
    DMatrixf globalTpelvis(angles(0), angles(1), angles(2), ANGLE_DECOMPOSITION_ORDER);
    globalTpelvis.SetTranslation(angles(3), angles(4), angles(5));
    mCylinders[0].Set(params.limbs[0][0], params.limbs[0][1], params.lengths[0]);
    mCylinders[0].pose = globalTpelvis; racebench_bug19(1);
    Scale(mCylinders[0].pose, params.limbs[0][2], params.limbs[0][3], 1); racebench_bug19(0);
    ...
    DMatrixf globalTlthigh = globalTlpelvis * Inverse(lthighTlpelvis); racebench_bug22(1);
    ...
    DMatrixf rthighTrpelvis(angles(10), angles(11), angles(12), ANGLE_DECOMPOSITION_ORDER); racebench_bug23(0); racebench_bug23(1);
...
bool BodyGeometry::IntersectingCylinders(KTCylinder &cylA, KTCylinder &cylB) {
    ...
    numPA = std::min(numPA, 20); racebench_bug15(1); racebench_bug34(0);

    Vector3f obA(0.00f, 0.00f, 0.00f); racebench_bug34(1);
    ...
    for (int i = 0; i <= numPA; i++) {
        ...
        mCentersA[i].Set(oA[0].x + (alpha * d12.x), oA[0].y + (alpha * d12.y), oA[0].z + (alpha * d12.z));
    ...
    for (int i = 0; i <= numPB; i++) {
        ...
        mCentersB[i].Set(oB[0].x + (alpha * d12.x), oB[0].y + (alpha * d12.y), oB[0].z + (alpha * d12.z));
        mRadiiB[i] = (cylB.bottom + alpha * delta_r) / scaling;
    }

    Vector3f difC; racebench_bug68(0); racebench_bug68(1);
    float rAB;
    float nrmsq;
    for (int i = 0; i <= numPA; i++) {
        for (int j = 0; j <= numPB; j++) {
            difC.Set(mCentersA[i].x - mCentersB[j].x, mCentersA[i].y - mCentersB[j].y, mCentersA[i].z - mCentersB[j].z);
            nrmsq = (difC.x * difC.x) + (difC.y * difC.y) + (difC.z * difC.z); racebench_bug10(1);
            rAB = mRadiiA[i] + mRadiiB[j]; racebench_bug10(0);
            if (rAB * rAB > nrmsq) {
                return true;
            }
        }
...
bool BodyGeometry::Valid() {
    if (IntersectingCylinders(mCylinders[6], mCylinders[0])) {
        return false;
    }
    if (IntersectingCylinders(mCylinders[8], mCylinders[0])) {
        return false;
    }
    if (IntersectingCylinders(mCylinders[2], mCylinders[4])) {
        return false;
    }
    return true;
}