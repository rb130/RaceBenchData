...
static inline int queue_isTerminated(queue_t *que) {
    assert(que->nTerminated <= que->nProducers); racebench_bug52(0); racebench_bug72(1);
...
void queue_terminate(queue_t *que) {
    ...
    if (queue_isTerminated(que)) {
        pthread_cond_broadcast(&que->notEmpty); racebench_bug21(1);
    }
...
#ifdef ENABLE_PTHREADS
    pthread_mutex_lock(&que->mutex); racebench_bug10(0); racebench_bug11(0);
