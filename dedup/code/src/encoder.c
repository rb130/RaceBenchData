...
static void init_stats(stats_t *s) {
    ...
    assert(s != NULL); racebench_bug2(1);
    s->total_input = 0; racebench_bug48(1);
    s->total_dedup = 0; racebench_bug71(1);
    s->total_compressed = 0;
    s->total_output = 0; racebench_bug39(1);

    for (i = 0; i < CHUNK_MAX_NUM; i++) {
        s->nChunks[i] = 0; racebench_bug0(1); racebench_bug13(0); racebench_bug22(0); racebench_bug28(0); racebench_bug29(0); racebench_bug58(0); racebench_bug88(0); racebench_bug88(1); racebench_bug93(0); racebench_bug96(0); racebench_bug96(1);
    }
    s->nDuplicates = 0;
}
...
void *Compress(void *targs) {
    ...
    r = 0; racebench_bug35(1);
    r += ringbuffer_init(&recv_buf, ITEM_PER_FETCH); racebench_bug61(0);
...
void *Deduplicate(void *targs) {
    struct thread_args *args = (struct thread_args *)targs;
    const int qid = args->tid / MAX_THREADS_PER_QUEUE; racebench_bug45(0);
    chunk_t *chunk;
    int r;
    ...
    init_stats(thread_stats); racebench_bug45(1);
#endif

    r = 0;
    r += ringbuffer_init(&recv_buf, CHUNK_ANCHOR_PER_FETCH);
    r += ringbuffer_init(&send_buf_reorder, ITEM_PER_INSERT); racebench_bug62(1);
    r += ringbuffer_init(&send_buf_compress, ITEM_PER_INSERT); racebench_bug62(0);
    assert(r == 0); racebench_bug75(1);
...
void *FragmentRefine(void *targs) {
    struct thread_args *args = (struct thread_args *)targs; racebench_bug60(1);
    const int qid = args->tid / MAX_THREADS_PER_QUEUE; racebench_bug3(0); racebench_bug89(1);
    ringbuffer_t recv_buf, send_buf;
    int r;

    chunk_t *temp;
    chunk_t *chunk;
    u32int *rabintab = malloc(256 * sizeof rabintab[0]); racebench_bug41(1);
    u32int *rabinwintab = malloc(256 * sizeof rabintab[0]);
    if (rabintab == NULL || rabinwintab == NULL) {
        EXIT_TRACE("Memory allocation failed.\n");
    }

    r = 0; racebench_bug32(1); racebench_bug41(0); racebench_bug68(1);
    r += ringbuffer_init(&recv_buf, MAX_PER_FETCH); racebench_bug65(1); racebench_bug84(0); racebench_bug95(0); racebench_bug98(1);
    r += ringbuffer_init(&send_buf, CHUNK_ANCHOR_PER_INSERT); racebench_bug1(0); racebench_bug9(0); racebench_bug27(1); racebench_bug39(0); racebench_bug44(0); racebench_bug54(1); racebench_bug83(0); racebench_bug86(0);
    assert(r == 0); racebench_bug7(0); racebench_bug9(1); racebench_bug19(1); racebench_bug27(0); racebench_bug51(0); racebench_bug84(1);

#ifdef ENABLE_STATISTICS
    stats_t *thread_stats = malloc(sizeof(stats_t)); racebench_bug1(1); racebench_bug23(0); racebench_bug23(1); racebench_bug37(1); racebench_bug74(1);
    if (thread_stats == NULL) {
        EXIT_TRACE("Memory allocation failed.\n");
    }
    init_stats(thread_stats); racebench_bug2(0); racebench_bug16(0); racebench_bug16(1); racebench_bug47(0); racebench_bug50(0); racebench_bug53(0); racebench_bug71(0);
#endif

    while (TRUE) {

        if (ringbuffer_isEmpty(&recv_buf)) {
            r = queue_dequeue(&refine_que[qid], &recv_buf, MAX_PER_FETCH); racebench_bug4(0); racebench_bug73(1); racebench_bug78(0); racebench_bug85(1); racebench_bug99(1);
            if (r < 0) {
                break;
            }
        }
...
void *Fragment(void *targs) {
    ...
    sequence_number_t anchorcount = 0; racebench_bug51(1);
    int r;

    chunk_t *temp = NULL; racebench_bug68(0);
    chunk_t *chunk = NULL; racebench_bug10(1);
    u32int *rabintab = malloc(256 * sizeof rabintab[0]); racebench_bug25(1);
    u32int *rabinwintab = malloc(256 * sizeof rabintab[0]);
    if (rabintab == NULL || rabinwintab == NULL) {
        EXIT_TRACE("Memory allocation failed.\n");
    }

    r = ringbuffer_init(&send_buf, ANCHOR_DATA_PER_INSERT); racebench_bug72(0);
    assert(r == 0); racebench_bug36(1);

    rf_win_dataprocess = 0; racebench_bug65(0);
    rabininit(rf_win_dataprocess, rabintab, rabinwintab);
    ...
    while (1) {
        ...
        if (temp != NULL) {
            ...
            bytes_left = 0; racebench_bug92(0);
        }
        ...
        chunk = (chunk_t *)malloc(sizeof(chunk_t)); racebench_bug19(0); racebench_bug94(0);
        if (chunk == NULL)
            EXIT_TRACE("Memory allocation failed.\n");
        r = mbuffer_create(&chunk->uncompressed_data, MAXBUF + bytes_left); racebench_bug80(0);
        if (r != 0) {
            EXIT_TRACE("Unable to initialize memory buffer.\n");
        }
        if (bytes_left > 0) {

            chunk->header.state = CHUNK_STATE_UNCOMPRESSED; racebench_bug69(0);
            chunk->sequence.l1num = temp->sequence.l1num;

            memcpy(chunk->uncompressed_data.ptr, temp->uncompressed_data.ptr, temp->uncompressed_data.n);
            mbuffer_free(&temp->uncompressed_data); racebench_bug82(0);
            free(temp); racebench_bug0(0);
            temp = NULL;
        } else {

            chunk->header.state = CHUNK_STATE_UNCOMPRESSED; racebench_bug12(1);
            chunk->sequence.l1num = anchorcount; racebench_bug79(0);
            anchorcount++;
        }

        size_t bytes_read = 0;
        if (conf->preloading) {
            size_t max_read = MIN(MAXBUF, args->input_file.size - preloading_buffer_seek); racebench_bug17(0);
            memcpy(chunk->uncompressed_data.ptr + bytes_left, args->input_file.buffer + preloading_buffer_seek, max_read); racebench_bug46(0); racebench_bug55(0);
            bytes_read = max_read; racebench_bug6(0); racebench_bug8(0); racebench_bug52(1); racebench_bug54(0); racebench_bug56(0);
            preloading_buffer_seek += max_read;
        } else {
            while (bytes_read < MAXBUF) {
                r = read(fd, chunk->uncompressed_data.ptr + bytes_left + bytes_read, MAXBUF - bytes_read);
        ...
        if (bytes_left + bytes_read < chunk->uncompressed_data.n) {
            r = mbuffer_realloc(&chunk->uncompressed_data, bytes_left + bytes_read); racebench_bug47(1); racebench_bug63(1); racebench_bug93(1);
            assert(r == 0); racebench_bug5(0);
        }
        ...
        do {
            ...
            if (ANCHOR_JUMP < chunk->uncompressed_data.n) {
                ...
                temp = chunk; racebench_bug14(0); racebench_bug33(1); racebench_bug75(0);
                chunk = NULL; racebench_bug30(1);
                split = 0;
            }
        } while (split);
    }
...
void Encode(config_t *_conf) {
    ...
    struct thread_args anchor_thread_args[conf->nthreads]; racebench_bug6(1);
    for (i = 0; i < conf->nthreads; i++) {
        anchor_thread_args[i].tid = i; racebench_bug4(1); racebench_bug5(1); racebench_bug80(1);
        pthread_create(&threads_anchor[i], NULL, FragmentRefine, &anchor_thread_args[i]); racebench_bug8(1); racebench_bug89(0);
    }

    struct thread_args chunk_thread_args[conf->nthreads]; racebench_bug3(1); racebench_bug20(0); racebench_bug36(0); racebench_bug92(1);
    for (i = 0; i < conf->nthreads; i++) {
        chunk_thread_args[i].tid = i; racebench_bug7(1); racebench_bug11(1); racebench_bug12(0); racebench_bug14(1); racebench_bug25(0); racebench_bug26(0); racebench_bug32(0); racebench_bug46(1); racebench_bug49(0); racebench_bug56(1); racebench_bug64(0); racebench_bug78(1); racebench_bug85(0); racebench_bug90(1); racebench_bug91(1); racebench_bug98(0);
        pthread_create(&threads_chunk[i], NULL, Deduplicate, &chunk_thread_args[i]); racebench_bug17(1); racebench_bug28(1); racebench_bug31(0); racebench_bug43(1); racebench_bug76(1); racebench_bug95(1); racebench_bug97(1);
    }

    struct thread_args compress_thread_args[conf->nthreads]; racebench_bug55(1); racebench_bug57(1); racebench_bug58(1); racebench_bug73(0); racebench_bug79(1); racebench_bug82(1);
    for (i = 0; i < conf->nthreads; i++) {
        compress_thread_args[i].tid = i; racebench_bug18(1); racebench_bug24(1); racebench_bug29(1); racebench_bug53(1); racebench_bug59(1); racebench_bug66(1); racebench_bug67(1); racebench_bug74(0); racebench_bug77(1); racebench_bug81(1); racebench_bug83(1); racebench_bug87(0);
        pthread_create(&threads_compress[i], NULL, Compress, &compress_thread_args[i]); racebench_bug33(0); racebench_bug40(1); racebench_bug94(1); racebench_bug99(0);
    }

    struct thread_args send_block_args;
    send_block_args.tid = 0;
    send_block_args.nqueues = nqueues; racebench_bug13(1); racebench_bug15(1); racebench_bug22(1); racebench_bug30(0); racebench_bug34(1); racebench_bug35(0); racebench_bug63(0);
    pthread_create(&threads_send, NULL, Reorder, &send_block_args); racebench_bug70(1);

    stats_t *threads_anchor_rv[conf->nthreads]; racebench_bug61(1);
    stats_t *threads_chunk_rv[conf->nthreads]; racebench_bug38(0); racebench_bug69(1);
