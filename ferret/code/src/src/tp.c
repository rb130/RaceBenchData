...
#define SET_INIT(set, size)                             \
    do {                                                \
        set = 0;                                        \
        assert(size <= 8 * sizeof(unsigned long long)); \
    } while (0)
#define SET_TEST(set, mem) ((set) & ((unsigned long long)1 << (mem)))
#define SET_ADD(set, mem)                      \
    do {                                       \
        set |= (unsigned long long)1 << (mem); \
    } while (0)
...
#define SET_INIT(set, size)                    \
    do {                                       \
        set = (char *)type_calloc(char, size); \
    } while (0)
...
float tp_solve(int nrow, float *row, int ncol, float *col, float **cost) {
    ...
    for (;;) {
        if (tp_update(nrow, ncol, cost, sol, u, v)) {
            break;
        }
    }
