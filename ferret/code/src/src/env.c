...
int cass_env_open(cass_env_t **_env, char *base_dir, uint32_t flags) {
    ...
    if (mkpath(buf, base_dir, "__cass", "env") != 0) {
        goto err;
    }
    ...
    if (cass_read_size(&n, 1, in) != 1) {
        goto err;
    }
    ret = CASS_ERR_OUTOFMEM;
    cass_reg_init_size(&env->table, n);
    if (n != 0 && env->table.data == NULL) {
        goto err;
    }
    ...
    if (n != 0 && env->vecset_dist.data == NULL) {
        goto err;
    }
...
int cass_env_checkpoint(cass_env_t *env) {
    ...
    if (env->mode & CASS_READONLY) {
        goto err;
    }

#warning add file switch
    if (mkpath(buf, env->base_dir, "__cass", "env") != 0) {
        goto err;
    }
    ...
    if (cass_write_size(&env->table.len, 1, out) != 1) {
        goto err;
    }
    ...
    if (cass_write_size(&env->cfg.len, 1, out) != 1) {
        goto err;
    }
    ...
    if (cass_write_size(&env->map.len, 1, out) != 1) {
        goto err;
    }
    ...
    if (cass_write_size(&env->vecset_dist.len, 1, out) != 1) {
        goto err;
    }
    ...
    ARRAY_BEGIN_FOREACH(env->vecset_dist, cass_reg_entry_t reg) {
        ...
        assert(id >= 0);
        if (cass_write_int32(&id, 1, out) != 1) {
            goto err;
        }
    ...
    if (cass_write_size(&env->vec_dist.len, 1, out) != 1) {
        goto err;
    }
...
int __cass_table_ref(cass_table_t *table) {
    ...
    if (table->cfg_id >= (signed)env->cfg.len) {
        return CASS_ERR_CORRUPTED;
    }
    if (table->cfg_id >= 0) {
    ...
    if (table->parent_cfg_id >= (signed)env->cfg.len) {
        return CASS_ERR_CORRUPTED;
    }
    ...
    if (table->map_id >= (signed)env->map.len) {
        return CASS_ERR_CORRUPTED;
    }
    if (table->map_id >= 0) {
    ...
    if (table->children.size > 0 && table->children.data == NULL) {
...
int __cass_table_unref(cass_table_t *table) {
    if (table->cfg != NULL) {
        cass_vecset_cfg_free(table->cfg);
    }
    if (table->parent_cfg != NULL) {
        cass_vecset_cfg_free(table->parent_cfg);
    }
    if (table->map != NULL) {
        cass_map_free(table->map);
    }
...
int cass_vecset_cfg_checkpoint(cass_vecset_cfg_t *cfg, CASS_FILE *out) {
    ...
    if (cass_write_uint32(&cfg->vecset_type, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_uint32(&cfg->vec_type, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_size(&cfg->vec_size, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_size(&cfg->vec_dim, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_uint32(&cfg->flag, 1, out) != 1) {
        return CASS_ERR_IO;
    }
...
int cass_vecset_cfg_restore(cass_vecset_cfg_t **_cfg, CASS_FILE *out) {
    ...
    if (cass_read_uint32(&cfg->vecset_type, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_uint32(&cfg->vec_type, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_size(&cfg->vec_size, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_size(&cfg->vec_dim, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_uint32(&cfg->flag, 1, out) != 1) {
        return CASS_ERR_IO;
    }
