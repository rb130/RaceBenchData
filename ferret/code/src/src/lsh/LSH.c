...

{
    ...
    if (strstr(param, "-W") && strstr(param, "-recall")) {
        debug("-W and -recall conflicts.\n");
    }
...
int LSH_free_private(cass_table_t *table) {
    ...
    if (lsh->recall.table != NULL) {
        LSH_recall_cleanup(&lsh->recall);
    }
...
int LSH_restore_private(cass_table_t *table, CASS_FILE *fin) {
    ...
    if (recall) {
        if (LSH_recall_load(&lsh->recall, fin) != 0) {
            assert(0);
        }
    }
...
int LSH_checkpoint_private(cass_table_t *table, CASS_FILE *fout) {
    ...
    if (recall) {
        if (LSH_recall_dump(&lsh->recall, fout) != 0) {
            assert(0);
        }
    }

    if (lsh->hash != NULL && table->dirty) {
        return LSH_dump(table);
    }
...
int __LSH_query(cass_table_t *table, cass_query_t *query, cass_result_t *result) {
    ...
    if (vec_dist->__class != &vec_dist_L2_float) {
        debug("LSH only works for L2 distance for float.\n");
    }

    if (table->parent_id == CASS_ID_INV) {
        debug("LSH query requires parent.\n");
    }
    parent = cass_reg_get(&table->env->table, table->parent_id);
    assert(parent != NULL);
    if (!parent->loaded) {
        debug("LSH query requires parent to be loaded.\n");
    }
    if (!(parent->opr->type & CASS_DATA)) {
        debug("LSH only works on CASS_DATA.\n");
    }
    ...
    K = query->topk;
    if (K == 0) {
        debug("LSH only works for KNN queries.\n");
    }
...
cass_table_opr_t opr_lsh = {.name = "lsh",
    .type = CASS_VEC_INDEX,
