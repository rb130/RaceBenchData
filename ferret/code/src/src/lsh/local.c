...
int LSH_est_init(LSH_est_t *est, int a_step, double a_min, double a_max, int b_step, double b_min, double b_max, double W, int M, int L, int K) {
    ...
    for (i = 0; i < a_step; i++) {
        for (j = 0; j < b_step; j++) {
            localdim(a_min + i * a_delta, b_min + j * b_delta, &est->a_table[i][j], &est->b_table[i][j], W, M, L, K);
        }
    }
