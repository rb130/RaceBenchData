...
int LSH_recall_init(LSH_recall_t *recall, int d_step, float d_min, float d_max, int M, int L, int T, float W) {
    ptb_vec_t *ptb_score;
    ptb_vec_t *ptb_set;
    float **table;
    double dist, r, p;
...
int LSH_recall_dump(LSH_recall_t *recall, CASS_FILE *fout) {
    ...
    if (type_matrix_dump_stream(float, fout, recall->T + 1, recall->d_step, recall->table) != 0) {
        return CASS_ERR_IO;
    }
