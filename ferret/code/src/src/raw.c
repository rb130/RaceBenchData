...
static int raw_query(cass_table_t *table, cass_query_t *query, cass_result_t *result) {
    ...
    vecset_dist = cass_reg_get(&table->env->vecset_dist, query->vecset_dist_id); racebench_bug92(0);
    assert(vecset_dist != NULL);

    result->flags = CASS_RESULT_LIST | CASS_RESULT_DIST;

    if (query->flags & CASS_RESULT_USERMEM) {
        if ((query->flags & CASS_RESULT_BITMAP) || ((query->flags & CASS_RESULT_LIST) == 0)) {
            debug("User allocated bitmap not supported.\n");
        }
    ...
    } else if (result->u.list.size > orig_size) {
        result->flags |= CASS_RESULT_REALLOC;
    }
...
cass_table_opr_t opr_raw = {.name = "raw",
