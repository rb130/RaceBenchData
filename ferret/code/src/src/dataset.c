...
int cass_dataset_checkpoint(cass_dataset_t *ds, CASS_FILE *out) {
    if (cass_write_uint32(&ds->flags, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_size(&ds->vec_size, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_size(&ds->vec_dim, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_size(&ds->num_vec, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_size(&ds->num_vecset, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    return 0;
}
...
int cass_dataset_restore(cass_dataset_t *ds, CASS_FILE *in) {
    memset(ds, 0, sizeof *ds);
    if (cass_read_uint32(&ds->flags, 1, in) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_size(&ds->vec_size, 1, in) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_size(&ds->vec_dim, 1, in) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_size(&ds->num_vec, 1, in) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_read_size(&ds->num_vecset, 1, in) != 1) {
        return CASS_ERR_IO;
    }
    return 0;
}
...
int cass_dataset_load(cass_dataset_t *ds, CASS_FILE *in, cass_vec_type_t vec_type) {
    ...
    if (ds->flags & CASS_DATASET_VECSET) {
        ...
        ret = CASS_ERR_IO;
        if (cass_read_vecset(ds->vecset, ds->num_vecset, in) != ds->num_vecset) {
            goto err;
        }
    }

    if (ds->flags & CASS_DATASET_VEC) {
        ds->max_vec = ds->num_vec;
        ret = CASS_ERR_OUTOFMEM;
...
int cass_dataset_dump(cass_dataset_t *ds, CASS_FILE *out) {
    ...
    if (ds->flags & CASS_DATASET_VECSET) {
        if (cass_write(ds->vecset, sizeof(cass_vecset_t), ds->num_vecset, out) != ds->num_vecset) {
            return CASS_ERR_IO;
        }
    }

    if (ds->flags & CASS_DATASET_VEC) {
        if (cass_write(ds->vec, ds->vec_size, ds->num_vec, out) != ds->num_vec) {
            return CASS_ERR_IO;
        }
    }
