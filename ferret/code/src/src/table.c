...
int cass_table_create(cass_table_t **_table, cass_env_t *env, char *table_name, uint32_t opr_idx, int32_t cfg_id, int32_t parent_id, int32_t parent_cfg_id, int32_t map_id, char *param) {
    char buf[BUFSIZ];
    int ret;
...
err:
    if (table != NULL) {
        cass_table_free(table);
    }
    return ret;
}

int cass_table_associate(cass_table_t *table, int32_t tbl2) {
...
int cass_table_disassociate(cass_table_t *table, int32_t tbl2) {
    int i;
    cass_table_t *ptbl2 = cass_reg_get(&table->env->table, tbl2);
    assert(ptbl2 != NULL);
    for (i = 0; i < table->children.len; i++) {
        if (table->children.data[i] == ptbl2) {
            break;
        }
    }
...
int cass_table_free(cass_table_t *table) {
    ...
    if (table->__private != NULL) {
        table->opr->free_private(table);
    }
...
int cass_table_restore(cass_table_t **_table, cass_env_t *env, CASS_FILE *in) {
    ...
    if (cass_read_int32(&table->parent_id, 1, in) != 1) {
        goto err;
    }
    if (cass_read_int32(&table->cfg_id, 1, in) != 1) {
        goto err;
    }
    if (cass_read_int32(&table->parent_cfg_id, 1, in) != 1) {
        goto err;
    }
    if (cass_read_int32(&table->map_id, 1, in) != 1) {
        goto err;
    }
    ...
    if (n != 0 && table->child_ids.data == NULL) {
        goto err;
    }

    ret = CASS_ERR_IO;
    if (n > 0) {
        if (cass_read_int32((int32_t *)table->child_ids.data, n, in) != n) {
            goto err;
        }
        table->child_ids.len = n;
    }

    if (cass_read_uint32(&c, 1, in) != 1) {
        goto err;
    }
...
int cass_table_checkpoint(cass_table_t *table, CASS_FILE *out) {
    uint32_t c;
    int ret;
    if (cass_write_pchar(table->name, out) != 0) {
        return CASS_ERR_IO;
    }

    if (cass_write_int32(&table->parent_id, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_int32(&table->cfg_id, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_int32(&table->parent_cfg_id, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_int32(&table->map_id, 1, out) != 1) {
        return CASS_ERR_IO;
    }

    if (cass_write_size(&table->child_ids.len, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_int32(table->child_ids.data, table->child_ids.len, out) != table->children.len) {
        return CASS_ERR_IO;
    }

    c = cass_table_opr_find(table->opr);
    if (cass_write_uint32(&c, 1, out) != 1) {
        return CASS_ERR_IO;
    }
...
int cass_table_query(cass_table_t *table, cass_query_t *query, cass_result_t *result) {
    if (table->opr->query != NULL) {
        return table->opr->query(table, query, result);
    } else {
...
int cass_table_batch_query(cass_table_t *table, uint32_t count, cass_query_t **queries, cass_result_t **results) {
    if (table->opr->batch_query != NULL) {
        return table->opr->batch_query(table, count, queries, results);
    } else {
        int i;
