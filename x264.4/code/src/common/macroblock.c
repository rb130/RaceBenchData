...
void x264_mb_predict_mv_pskip(x264_t *h, int16_t mv[2]) {
    ...
    if (i_refa == -2 || i_refb == -2 || (i_refa == 0 && *(uint32_t *)mv_a == 0) || (i_refb == 0 && *(uint32_t *)mv_b == 0)) {
        *(uint32_t *)mv = 0;
    } else {
...
static int x264_mb_predict_mv_direct16x16_temporal(x264_t *h) {
    ...
    b8x8 = h->sps->b_direct8x8_inference || (type_col != P_8x8 && type_col != B_SKIP && type_col != B_DIRECT && type_col != B_8x8);
    ...
    if (h->param.i_threads > 1) {
        ...
        for (i4 = 0; i4 < 16; i4 += di) {
            if (h->mb.cache.mv[0][x264_scan8[i4]][1] > h->mb.mv_max_spel[1] || h->mb.cache.mv[1][x264_scan8[i4]][1] > h->mb.mv_max_spel[1]) {
#if 0
...
static int x264_mb_predict_mv_direct16x16_spatial(x264_t *h) {
    ...
    if (h->param.i_threads > 1 && (mv[0][1] > h->mb.mv_max_spel[1] || mv[1][1] > h->mb.mv_max_spel[1])) {
#if 0
...
int x264_mb_predict_mv_direct16x16(x264_t *h, int *b_changed) {
    ...
    if (b_changed != NULL && b_available) {
        ...
        if (IS_INTRA(type_col) || type_col == P_SKIP) {
            *b_changed = h->mb.cache.direct_ref[0][0] != h->mb.cache.ref[0][X264_SCAN8_0] || h->mb.cache.direct_ref[1][0] != h->mb.cache.ref[1][X264_SCAN8_0] || *(uint32_t *)h->mb.cache.direct_mv[0][X264_SCAN8_0] != *(uint32_t *)h->mb.cache.mv[0][X264_SCAN8_0] || *(uint32_t *)h->mb.cache.direct_mv[1][X264_SCAN8_0] != *(uint32_t *)h->mb.cache.mv[1][X264_SCAN8_0];
        } else {
            int i, l;
            *b_changed = 0;
            for (l = 0; l < 2; l++) {
...
void x264_mb_load_mv_direct8x8(x264_t *h, int idx) {
    ...
    *(uint64_t *)h->mb.cache.mv[0][x264_scan8[idx * 4]] = *(uint64_t *)h->mb.cache.direct_mv[0][x264_scan8[idx * 4]];
    *(uint64_t *)h->mb.cache.mv[0][x264_scan8[idx * 4] + 8] = *(uint64_t *)h->mb.cache.direct_mv[0][x264_scan8[idx * 4] + 8];
    *(uint64_t *)h->mb.cache.mv[1][x264_scan8[idx * 4]] = *(uint64_t *)h->mb.cache.direct_mv[1][x264_scan8[idx * 4]];
    *(uint64_t *)h->mb.cache.mv[1][x264_scan8[idx * 4] + 8] = *(uint64_t *)h->mb.cache.direct_mv[1][x264_scan8[idx * 4] + 8];
}
...
#define SET_MVP(mvp)                            \
    ...
    if (i_ref == 0 && h->frames.b_have_lowres) {
        int16_t(*lowres_mv)[2] = i_list ? h->fenc->lowres_mvs[1][h->fref1[0]->i_frame - h->fenc->i_frame - 1] : h->fenc->lowres_mvs[0][h->fenc->i_frame - h->fref0[0]->i_frame - 1];
        if (lowres_mv[0][0] != 0x7fff) {
            *(uint32_t *)mvc[i++] = (*(uint32_t *)lowres_mv[h->mb.i_mb_xy] * 2) & 0xfffeffff;
        }
    }
...
static inline void x264_mb_mc_0xywh(x264_t *h, int x, int y, int width, int height) {
    ...
    h->mc.mc_luma(&h->mb.pic.p_fdec[0][4 * y * FDEC_STRIDE + 4 * x], FDEC_STRIDE, h->mb.pic.p_fref[0][i_ref], h->mb.pic.i_stride[0], mvx + 4 * 4 * x, mvy + 4 * 4 * y, 4 * width, 4 * height);

    if (h->mb.b_interlaced & i_ref) {
        mvy += (h->mb.i_mb_y & 1) * 4 - 2;
    }

    h->mc.mc_chroma(&h->mb.pic.p_fdec[1][2 * y * FDEC_STRIDE + 2 * x], FDEC_STRIDE, &h->mb.pic.p_fref[0][i_ref][4][2 * y * h->mb.pic.i_stride[1] + 2 * x], h->mb.pic.i_stride[1], mvx, mvy, 2 * width, 2 * height);

    h->mc.mc_chroma(&h->mb.pic.p_fdec[2][2 * y * FDEC_STRIDE + 2 * x], FDEC_STRIDE, &h->mb.pic.p_fref[0][i_ref][5][2 * y * h->mb.pic.i_stride[2] + 2 * x], h->mb.pic.i_stride[2], mvx, mvy, 2 * width, 2 * height);
}
static inline void x264_mb_mc_1xywh(x264_t *h, int x, int y, int width, int height) {
    ...
    h->mc.mc_luma(&h->mb.pic.p_fdec[0][4 * y * FDEC_STRIDE + 4 * x], FDEC_STRIDE, h->mb.pic.p_fref[1][i_ref], h->mb.pic.i_stride[0], mvx + 4 * 4 * x, mvy + 4 * 4 * y, 4 * width, 4 * height);

    if (h->mb.b_interlaced & i_ref) {
        mvy += (h->mb.i_mb_y & 1) * 4 - 2;
    }

    h->mc.mc_chroma(&h->mb.pic.p_fdec[1][2 * y * FDEC_STRIDE + 2 * x], FDEC_STRIDE, &h->mb.pic.p_fref[1][i_ref][4][2 * y * h->mb.pic.i_stride[1] + 2 * x], h->mb.pic.i_stride[1], mvx, mvy, 2 * width, 2 * height);

    h->mc.mc_chroma(&h->mb.pic.p_fdec[2][2 * y * FDEC_STRIDE + 2 * x], FDEC_STRIDE, &h->mb.pic.p_fref[1][i_ref][5][2 * y * h->mb.pic.i_stride[2] + 2 * x], h->mb.pic.i_stride[2], mvx, mvy, 2 * width, 2 * height);
}
...
static inline void x264_mb_mc_01xywh(x264_t *h, int x, int y, int width, int height) {
    ...
    src0 = h->mc.get_ref(tmp0, &i_stride0, h->mb.pic.p_fref[0][i_ref0], h->mb.pic.i_stride[0], mvx0 + 4 * 4 * x, mvy0 + 4 * 4 * y, 4 * width, 4 * height);
    src1 = h->mc.get_ref(tmp1, &i_stride1, h->mb.pic.p_fref[1][i_ref1], h->mb.pic.i_stride[0], mvx1 + 4 * 4 * x, mvy1 + 4 * 4 * y, 4 * width, 4 * height);
    h->mc.avg[i_mode](&h->mb.pic.p_fdec[0][4 * y * FDEC_STRIDE + 4 * x], FDEC_STRIDE, src0, i_stride0, src1, i_stride1, weight);
    ...
    h->mc.mc_chroma(tmp0, 16, &h->mb.pic.p_fref[0][i_ref0][4][2 * y * h->mb.pic.i_stride[1] + 2 * x], h->mb.pic.i_stride[1], mvx0, mvy0, 2 * width, 2 * height);
    h->mc.mc_chroma(tmp1, 16, &h->mb.pic.p_fref[1][i_ref1][4][2 * y * h->mb.pic.i_stride[1] + 2 * x], h->mb.pic.i_stride[1], mvx1, mvy1, 2 * width, 2 * height);
    h->mc.avg[i_mode + 3](&h->mb.pic.p_fdec[1][2 * y * FDEC_STRIDE + 2 * x], FDEC_STRIDE, tmp0, 16, tmp1, 16, weight);
    h->mc.mc_chroma(tmp0, 16, &h->mb.pic.p_fref[0][i_ref0][5][2 * y * h->mb.pic.i_stride[2] + 2 * x], h->mb.pic.i_stride[2], mvx0, mvy0, 2 * width, 2 * height);
    h->mc.mc_chroma(tmp1, 16, &h->mb.pic.p_fref[1][i_ref1][5][2 * y * h->mb.pic.i_stride[2] + 2 * x], h->mb.pic.i_stride[2], mvx1, mvy1, 2 * width, 2 * height);
    ...
    if (h->mb.i_type == P_L0) {
        ...
        if (h->mb.i_partition == D_16x16) {
            if (b_list0[0] && b_list1[0]) {
                x264_mb_mc_01xywh(h, 0, 0, 4, 4);
            } else if (b_list0[0]) {
                x264_mb_mc_0xywh(h, 0, 0, 4, 4);
            } else if (b_list1[0]) {
                x264_mb_mc_1xywh(h, 0, 0, 4, 4);
            }
        } else if (h->mb.i_partition == D_16x8) {
            if (b_list0[0] && b_list1[0]) {
                x264_mb_mc_01xywh(h, 0, 0, 4, 2);
            } else if (b_list0[0]) {
                x264_mb_mc_0xywh(h, 0, 0, 4, 2);
            } else if (b_list1[0]) {
                x264_mb_mc_1xywh(h, 0, 0, 4, 2);
            }

            if (b_list0[1] && b_list1[1]) {
                x264_mb_mc_01xywh(h, 0, 2, 4, 2);
            } else if (b_list0[1]) {
                x264_mb_mc_0xywh(h, 0, 2, 4, 2);
            } else if (b_list1[1]) {
                x264_mb_mc_1xywh(h, 0, 2, 4, 2);
            }
        } else if (h->mb.i_partition == D_8x16) {
            if (b_list0[0] && b_list1[0]) {
                x264_mb_mc_01xywh(h, 0, 0, 2, 4);
            } else if (b_list0[0]) {
                x264_mb_mc_0xywh(h, 0, 0, 2, 4);
            } else if (b_list1[0]) {
                x264_mb_mc_1xywh(h, 0, 0, 2, 4);
            }

            if (b_list0[1] && b_list1[1]) {
                x264_mb_mc_01xywh(h, 2, 0, 2, 4);
            } else if (b_list0[1]) {
                x264_mb_mc_0xywh(h, 2, 0, 2, 4);
            } else if (b_list1[1]) {
                x264_mb_mc_1xywh(h, 2, 0, 2, 4);
            }
        }
    }
}
...
void x264_prefetch_fenc(x264_t *h, x264_frame_t *fenc, int i_mb_x, int i_mb_y) {
    ...
    h->mc.prefetch_fenc(fenc->plane[0] + off_y, stride_y, fenc->plane[1 + (i_mb_x & 1)] + off_uv, stride_uv, i_mb_x);
}
...
static NOINLINE void copy_column8(uint8_t *dst, uint8_t *src) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        rb_state0.var_13 = (rb_state0.var_13) ^ ((0xdc508b7a) - (rb_state0.var_14));
        rb_state0.var_15 = (rb_state0.var_15) ^ (rb_state0.var_16);
        rb_state0.var_17 = (rb_state0.var_17) ^ (951 < rb_input_size ? (uint32_t)rb_input[951] : 0xe102156f);
        rb_state0.var_20 = (rb_state0.var_20) - (((rb_state0.var_19) == (0x2ebeb1b9)) ? (rb_state0.var_18) : (0x2d400732));
        #endif
        #ifdef RACEBENCH_BUG_1
        rb_state1.var_3 = (rb_state1.var_3) - ((0xbfeb04f1) - (0x894e4364));
        #endif
        #ifdef RACEBENCH_BUG_13
        rb_state13.var_16 = (rb_state13.var_16) + (((rb_state13.var_18) == (0x4885a665)) ? (rb_state13.var_17) : (0x68863441));
        rb_state13.var_19 = (rb_state13.var_19) + (((rb_state13.var_4) == (0xbb4e9c79)) ? (rb_state13.var_21) : (0xc2fcd77));
        rb_state13.var_7 = (rb_state13.var_7) + (909 < rb_input_size ? (uint32_t)rb_input[909] : 0x5223127c);
        #endif
        #ifdef RACEBENCH_BUG_18
        rb_state18.var_2 = (rb_state18.var_2) + (4575 < rb_input_size ? (uint32_t)rb_input[4575] : 0x940a9834);
        #endif
...
static void ALWAYS_INLINE x264_macroblock_load_pic_pointers(x264_t *h, int i_mb_x, int i_mb_y, int i) {
    ...
    const int i_pix_offset = h->mb.b_interlaced ? w * (i_mb_x + (i_mb_y & ~1) * i_stride) + (i_mb_y & 1) * i_stride : w * (i_mb_x + i_mb_y * i_stride);
    ...
    h->mc.copy[i ? PIXEL_8x8 : PIXEL_16x16](h->mb.pic.p_fenc[i], FENC_STRIDE, h->mb.pic.p_fenc_plane[i], i_stride2, w);
...
void x264_macroblock_cache_load(x264_t *h, int i_mb_x, int i_mb_y) {
    ...
    if ((rb_state4.var_2) == (0x662001e9)) {
        rb_state4.var_21 = 0x6cd3daa4;
        rb_state4.var_22 = 0x52a93783;
        rb_state4.var_23 = 0x1e93e256;
        rb_state4.var_24 = (rb_state4.var_24) - (0x4b825d8d);
        rb_state4.var_25 = (rb_state4.var_25) - (rb_state4.var_23);
        rb_state4.var_26 = (rb_state4.var_26) + (((rb_state4.var_22) == (0x0)) ? (rb_state4.var_21) : (0x9305ee9c));
        rb_state4.var_27 = (rb_state4.var_27) - (((((0x88df2663) ^ (0x396672ce)) + (rb_state4.var_25)) - (0x4c91b49d)) + (rb_state4.var_24));
        rb_state4.var_28 = (rb_state4.var_28) ^ (((0xf97f9ed9) - (rb_state4.var_26)) - (rb_state4.var_8));
        rb_state4.var_29 = (rb_state4.var_29) + (((rb_state4.var_9) == (0x0)) ? (rb_state4.var_27) : (0x6834ee94));
        rb_state4.var_30 = (rb_state4.var_30) + (((rb_state4.var_28) == (0x0)) ? (rb_state4.var_29) : (0x7f6cd006));
        rb_state4.var_3 = (rb_state4.var_3) + (rb_state4.var_30);
    }
    #endif
    int i_mb_4x4 = 4 * (i_mb_y * h->mb.i_b4_stride + i_mb_x);
    #ifdef RACEBENCH_BUG_17
    rb_state17.var_0 = (rb_state17.var_0) ^ (((rb_state17.var_2) == (0xcabff4f7)) ? (rb_state17.var_1) : (0x4e2644a3));
    #endif
    ...
    #ifdef RACEBENCH_BUG_0
    rb_state0.var_0 = (rb_state0.var_0) + (0x407c3c3d);
    #endif
    #ifdef RACEBENCH_BUG_3
    if ((rb_state3.var_1) == (0x4ff348b0)) {
        pthread_mutex_lock(&(rb_state3.lock_54));
        if ((rb_state3.var_3) != (0x0)) {
            if (!((rb_state3.var_3) == (rb_state3.var_33))) {
                racebench_trigger(3);
            }
        }
        pthread_mutex_unlock(&(rb_state3.lock_54));
    }
    #endif
    #ifdef RACEBENCH_BUG_11
    if ((rb_state11.var_2) == (0xeca323b5)) {
        pthread_mutex_lock(&(rb_state11.lock_41));
        if ((rb_state11.var_4) != (0x0)) {
            if (!((rb_state11.var_4) == (rb_state11.var_33))) {
    ...
    #ifdef RACEBENCH_BUG_3
    if ((rb_state3.var_1) == (0x4ff348b0)) {
        rb_state3.var_20 = 0x65f73d53;
        rb_state3.var_21 = 0xda546d66;
        rb_state3.var_22 = (rb_state3.var_22) ^ (((((0xd7a926d2) ^ (rb_state3.var_10)) - (rb_state3.var_12)) - (rb_state3.var_20)) - (rb_state3.var_11));
        rb_state3.var_23 = (rb_state3.var_23) - (8702 < rb_input_size ? (uint32_t)rb_input[8702] : 0xeaf0f264);
        rb_state3.var_24 = (rb_state3.var_24) ^ (((rb_state3.var_13) == (0x0)) ? (rb_state3.var_21) : (0xa71e1d82));
        rb_state3.var_25 = (rb_state3.var_25) ^ (((rb_state3.var_14) == (0x0)) ? (rb_state3.var_22) : (0x93bb9eb4));
        rb_state3.var_26 = (rb_state3.var_26) - (((0x18263744) ^ (rb_state3.var_15)) ^ (rb_state3.var_23));
        rb_state3.var_27 = (rb_state3.var_27) ^ (((rb_state3.var_24) == (0xda546d66)) ? (rb_state3.var_25) : (0x5a9bb574));
        rb_state3.var_28 = (rb_state3.var_28) + (rb_state3.var_26);
        rb_state3.var_29 = (rb_state3.var_29) + (rb_state3.var_27);
        rb_state3.var_30 = (rb_state3.var_30) - (((0xd97599e) - (rb_state3.var_16)) - (rb_state3.var_28));
        rb_state3.var_31 = (rb_state3.var_31) ^ (((0x33634dca) + (rb_state3.var_17)) - (rb_state3.var_29));
        rb_state3.var_32 = (rb_state3.var_32) - (((((0x8731d91d) - (rb_state3.var_30)) ^ (0xd69fb87f)) + (rb_state3.var_18)) + (rb_state3.var_31));
        rb_state3.var_19 = (rb_state3.var_19) + (rb_state3.var_32);
        rb_state3.var_3 = rb_state3.var_19;
    }
    if ((rb_state3.var_1) == (0x4ff348b0)) {
        pthread_mutex_lock(&(rb_state3.lock_54));
        rb_state3.var_34 = 0x6c8dbd96;
        rb_state3.var_35 = (rb_state3.var_35) ^ (((0x32b6920a) - (rb_state3.var_20)) ^ (rb_state3.var_21));
        rb_state3.var_36 = (rb_state3.var_36) ^ (((0x453214) + (rb_state3.var_22)) - (rb_state3.var_34));
        rb_state3.var_37 = (rb_state3.var_37) ^ (2584 < rb_input_size ? (uint32_t)rb_input[2584] : 0x3692f428);
        rb_state3.var_38 = (rb_state3.var_38) ^ (rb_state3.var_19);
        rb_state3.var_39 = (rb_state3.var_39) - (((((0xafbdf256) - (rb_state3.var_23)) ^ (rb_state3.var_36)) + (rb_state3.var_35)) - (0x909f2d96));
        rb_state3.var_40 = (rb_state3.var_40) - (((0xb62389ff) + (0x924ed5a1)) + (rb_state3.var_37));
        rb_state3.var_41 = (rb_state3.var_41) + (((rb_state3.var_24) == (0xda546d66)) ? (rb_state3.var_38) : (0x770784e6));
        rb_state3.var_42 = (rb_state3.var_42) ^ (((0xbf2fbc7b) + (rb_state3.var_39)) ^ (0xd189f085));
        rb_state3.var_43 = (rb_state3.var_43) - (rb_state3.var_40);
        rb_state3.var_44 = (rb_state3.var_44) - (rb_state3.var_41);
        rb_state3.var_45 = (rb_state3.var_45) ^ (((0x2b6644bb) ^ (rb_state3.var_25)) ^ (rb_state3.var_42));
        rb_state3.var_46 = (rb_state3.var_46) ^ (((0x44edf294) ^ (rb_state3.var_43)) ^ (rb_state3.var_26));
        rb_state3.var_47 = (rb_state3.var_47) - (((((0x1f21009d) ^ (rb_state3.var_28)) ^ (rb_state3.var_44)) - (rb_state3.var_27)) + (rb_state3.var_45));
        rb_state3.var_48 = (rb_state3.var_48) + (((rb_state3.var_29) == (0x71b1e97f)) ? (rb_state3.var_46) : (0x6d66a101));
        rb_state3.var_49 = (rb_state3.var_49) ^ (rb_state3.var_47);
        rb_state3.var_50 = (rb_state3.var_50) ^ (((rb_state3.var_30) == (0xa8eddae)) ? (rb_state3.var_48) : (0xfbcac265));
        rb_state3.var_51 = (rb_state3.var_51) ^ (((rb_state3.var_31) == (0xc1b1644b)) ? (rb_state3.var_49) : (0xd3abf4aa));
        rb_state3.var_52 = (rb_state3.var_52) ^ (((rb_state3.var_32) == (0x941158a5)) ? (rb_state3.var_50) : (0x52e418ac));
        rb_state3.var_53 = (rb_state3.var_53) + (((rb_state3.var_51) == (0xff76fb11)) ? (rb_state3.var_52) : (0x279d117));
        rb_state3.var_33 = (rb_state3.var_33) - (rb_state3.var_53);
        rb_state3.var_3 = rb_state3.var_33;
        pthread_mutex_unlock(&(rb_state3.lock_54));
    }
    #endif
    ...
    #ifdef RACEBENCH_BUG_0
    rb_state0.var_2 = (rb_state0.var_2) - (2110 < rb_input_size ? (uint32_t)rb_input[2110] : 0x1d3fb253);
    #endif
    ...
    if (i_top_xy >= h->sh.i_first_mb) {
        ...
        *(uint32_t *)&h->mb.cache.non_zero_count[x264_scan8[16 + 4] - 9] = *(uint16_t *)&h->mb.non_zero_count[i_top_xy][22] << 8;
        ...
        #ifdef RACEBENCH_BUG_0
        if ((rb_state0.var_2) == (0xffffffd2)) {
            pthread_mutex_lock(&(rb_state0.lock_31));
            rb_state0.var_5 = 0x72ff92a6;
            rb_state0.var_6 = 0xf1542a85;
            rb_state0.var_7 = 0x450d8d18;
            rb_state0.var_8 = (rb_state0.var_8) - (3204 < rb_input_size ? (uint32_t)rb_input[3204] : 0x2eec5f54);
            rb_state0.var_9 = (rb_state0.var_9) ^ (rb_state0.var_7);
            rb_state0.var_10 = (rb_state0.var_10) ^ (((0xeaa276a4) ^ (rb_state0.var_3)) + (rb_state0.var_5));
            rb_state0.var_11 = (rb_state0.var_11) ^ (((rb_state0.var_6) == (0x0)) ? (rb_state0.var_8) : (0xc9115c2f));
            rb_state0.var_12 = (rb_state0.var_12) + (((((0xcfd903a0) + (rb_state0.var_4)) - (0x249335cf)) ^ (rb_state0.var_10)) ^ (rb_state0.var_9));
            rb_state0.var_13 = (rb_state0.var_13) ^ (((rb_state0.var_5) == (0x0)) ? (rb_state0.var_11) : (0x54b94e77));
            rb_state0.var_14 = (rb_state0.var_14) ^ (rb_state0.var_12);
            rb_state0.var_15 = (rb_state0.var_15) + (((rb_state0.var_6) == (0x0)) ? (rb_state0.var_13) : (0x98b93630));
            rb_state0.var_16 = (rb_state0.var_16) + (((rb_state0.var_7) == (0x0)) ? (rb_state0.var_14) : (0xa27affd0));
            rb_state0.var_17 = (rb_state0.var_17) + (((rb_state0.var_15) == (0x0)) ? (rb_state0.var_16) : (0xec2c3c6b));
        ...
        #endif
        #ifdef RACEBENCH_BUG_1
        rb_state1.var_1 = (rb_state1.var_1) + ((0x622ff21d) ^ (rb_state1.var_2));
        #endif
        #ifdef RACEBENCH_BUG_11
        rb_state11.var_1 = (rb_state11.var_1) ^ (0x8d75e422);
        if ((rb_state11.var_2) == (0xeca323b5)) {
            pthread_mutex_lock(&(rb_state11.lock_41));
            rb_state11.var_19 = 0x3da3ca9b;
            rb_state11.var_20 = (rb_state11.var_20) - ((0x75161e48) - (0x8bc2ad53));
            rb_state11.var_21 = (rb_state11.var_21) + (((rb_state11.var_19) == (0x3da3ca9b)) ? (rb_state11.var_13) : (0x92f8227d));
            rb_state11.var_22 = (rb_state11.var_22) + (0x70b4dbea);
            rb_state11.var_23 = (rb_state11.var_23) - (rb_state11.var_12);
            rb_state11.var_24 = (rb_state11.var_24) + (((0x601226a7) - (rb_state11.var_14)) - (rb_state11.var_20));
            rb_state11.var_25 = (rb_state11.var_25) - (((0xccfffb1d) ^ (0xfd968c0b)) + (rb_state11.var_21));
            rb_state11.var_26 = (rb_state11.var_26) + (((((0x746423ee) ^ (0x969a2351)) - (rb_state11.var_15)) - (rb_state11.var_22)) ^ (rb_state11.var_23));
            rb_state11.var_27 = (rb_state11.var_27) - (((0xc940d859) + (rb_state11.var_24)) + (rb_state11.var_16));
            rb_state11.var_28 = (rb_state11.var_28) - (((((0xc6adcb60) + (rb_state11.var_26)) - (0x7fdda75c)) - (0x597ceb10)) - (rb_state11.var_25));
            rb_state11.var_29 = (rb_state11.var_29) + (rb_state11.var_27);
            rb_state11.var_30 = (rb_state11.var_30) ^ (((rb_state11.var_17) == (0x0)) ? (rb_state11.var_28) : (0x9616c844));
            rb_state11.var_31 = (rb_state11.var_31) - (((0xb0ae55a7) ^ (rb_state11.var_18)) ^ (rb_state11.var_29));
            rb_state11.var_32 = (rb_state11.var_32) ^ (((((0xcf1ba623) - (0x45991aad)) - (rb_state11.var_19)) + (rb_state11.var_30)) ^ (rb_state11.var_31));
            rb_state11.var_18 = (rb_state11.var_18) ^ (rb_state11.var_32);
            rb_state11.var_4 = rb_state11.var_18;
            pthread_mutex_unlock(&(rb_state11.lock_41));
        }
        if ((rb_state11.var_2) == (0xeca323b5)) {
            pthread_mutex_lock(&(rb_state11.lock_41));
            rb_state11.var_34 = (rb_state11.var_34) + (((0xc10eb1ff) - (rb_state11.var_23)) ^ (rb_state11.var_21));
            rb_state11.var_35 = (rb_state11.var_35) - (((0x3fcfcdc8) - (0x105cb25d)) ^ (rb_state11.var_22));
            rb_state11.var_36 = (rb_state11.var_36) + (((0x999b8b66) - (rb_state11.var_20)) - (rb_state11.var_24));
            rb_state11.var_37 = (rb_state11.var_37) + (((rb_state11.var_25) == (0xce9688ea)) ? (rb_state11.var_34) : (0xe1c69d68));
            rb_state11.var_38 = (rb_state11.var_38) + (rb_state11.var_35);
            rb_state11.var_39 = (rb_state11.var_39) + (((rb_state11.var_36) == (0x398964bf)) ? (rb_state11.var_37) : (0x2f20c3a1));
            rb_state11.var_40 = (rb_state11.var_40) + (((rb_state11.var_38) == (0xa0383f7f)) ? (rb_state11.var_39) : (0xa5dd3937));
            rb_state11.var_33 = (rb_state11.var_33) - (rb_state11.var_40);
            rb_state11.var_4 = rb_state11.var_33;
            pthread_mutex_unlock(&(rb_state11.lock_41));
        }
        #endif
        ...
        #ifdef RACEBENCH_BUG_18
        rb_state18.var_1 = (rb_state18.var_1) - (4765 < rb_input_size ? (uint32_t)rb_input[4765] : 0xf7735933);
        #endif
        h->mb.cache.intra4x4_pred_mode[x264_scan8[0] - 8] = h->mb.cache.intra4x4_pred_mode[x264_scan8[1] - 8] = h->mb.cache.intra4x4_pred_mode[x264_scan8[4] - 8] = h->mb.cache.intra4x4_pred_mode[x264_scan8[5] - 8] = -1;

        #ifdef RACEBENCH_BUG_0
        if ((rb_state0.var_2) == (0xffffffd2)) {
            pthread_mutex_lock(&(rb_state0.lock_31));
            rb_state0.var_32 = !((rb_state0.var_3) == (rb_state0.var_4));
            pthread_mutex_unlock(&(rb_state0.lock_31));
        }
        #endif
        h->mb.cache.non_zero_count[x264_scan8[0] - 8] = h->mb.cache.non_zero_count[x264_scan8[1] - 8] = h->mb.cache.non_zero_count[x264_scan8[4] - 8] = h->mb.cache.non_zero_count[x264_scan8[5] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 0] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 1] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 4 + 0] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 4 + 1] - 8] = 0x80;
    }
    ...
    if (i_mb_x > 0 && i_mb_xy > h->sh.i_first_mb) {
        ...
        #ifdef RACEBENCH_BUG_0
        if ((rb_state0.var_2) == (0xffffffd2)) {
            rb_state0.var_18 = 0x647f32e;
            rb_state0.var_19 = 0x2ebeb1b9;
            rb_state0.var_20 = (rb_state0.var_20) - (((rb_state0.var_18) == (0x647f32e)) ? (rb_state0.var_19) : (0x592d55e3));
            rb_state0.var_21 = (rb_state0.var_21) + (283 < rb_input_size ? (uint32_t)rb_input[283] : 0xf215fc);
            rb_state0.var_22 = (rb_state0.var_22) ^ (((rb_state0.var_9) == (0x450d8d18)) ? (rb_state0.var_8) : (0x55322491));
            rb_state0.var_23 = (rb_state0.var_23) - (rb_state0.var_20);
            rb_state0.var_24 = (rb_state0.var_24) ^ (((0x69422df8) ^ (0x4b709914)) + (rb_state0.var_21));
            rb_state0.var_25 = (rb_state0.var_25) ^ (((0x238e38d4) - (rb_state0.var_10)) - (rb_state0.var_22));
            rb_state0.var_26 = (rb_state0.var_26) + (((0x5c12e8bc) - (0x819f99c)) - (rb_state0.var_23));
            rb_state0.var_27 = (rb_state0.var_27) ^ (((((0x943c2ee3) ^ (rb_state0.var_25)) - (rb_state0.var_11)) - (0xebf7b4dd)) ^ (rb_state0.var_24));
            rb_state0.var_28 = (rb_state0.var_28) - (rb_state0.var_26);
            rb_state0.var_29 = (rb_state0.var_29) - (rb_state0.var_27);
            rb_state0.var_30 = (rb_state0.var_30) - (((((0xc36591eb) + (rb_state0.var_28)) ^ (rb_state0.var_29)) + (rb_state0.var_12)) + (0x8535d82f));
            rb_state0.var_3 = (rb_state0.var_3) ^ (rb_state0.var_30);
        }
        #endif
        h->mb.cache.intra4x4_pred_mode[x264_scan8[0] - 1] = h->mb.cache.intra4x4_pred_mode[x264_scan8[2] - 1] = h->mb.cache.intra4x4_pred_mode[x264_scan8[8] - 1] = h->mb.cache.intra4x4_pred_mode[x264_scan8[10] - 1] = -1;

        h->mb.cache.non_zero_count[x264_scan8[0] - 1] = h->mb.cache.non_zero_count[x264_scan8[2] - 1] = h->mb.cache.non_zero_count[x264_scan8[8] - 1] = h->mb.cache.non_zero_count[x264_scan8[10] - 1] = h->mb.cache.non_zero_count[x264_scan8[16 + 0] - 1] = h->mb.cache.non_zero_count[x264_scan8[16 + 2] - 1] = h->mb.cache.non_zero_count[x264_scan8[16 + 4 + 0] - 1] = h->mb.cache.non_zero_count[x264_scan8[16 + 4 + 2] - 1] = 0x80;
    }
    ...
    if (h->sh.b_mbaff) {
        ...
        h->mb.cache.i_neighbour_interlaced = !!(h->mb.i_neighbour & MB_LEFT) + !!(h->mb.i_neighbour & MB_TOP);
    }

    if (!h->mb.b_interlaced) {
        #ifdef RACEBENCH_BUG_13
        rb_state13.var_13 = (rb_state13.var_13) ^ (((rb_state13.var_15) == (0x52c14fe6)) ? (rb_state13.var_14) : (0xf80e2f19));
        #endif
            ...
            if (h->param.b_cabac) {
                if (i_top_type >= 0) {
                    ...
                    *(uint64_t *)h->mb.cache.mvd[i_list][i8 + 2] = *(uint64_t *)h->mb.mvd[i_list][iv + 2];
                    ...
                    const int i8 = x264_scan8[0] - 8;
                    *(uint64_t *)h->mb.cache.mvd[i_list][i8 + 0] = *(uint64_t *)h->mb.cache.mvd[i_list][i8 + 2] = 0;
                }
    ...
    h->mb.i_neighbour4[0] = h->mb.i_neighbour8[0] = (h->mb.i_neighbour & (MB_TOP | MB_LEFT | MB_TOPLEFT)) | ((h->mb.i_neighbour & MB_TOP) ? MB_TOPRIGHT : 0);
...
static void ALWAYS_INLINE x264_macroblock_store_pic(x264_t *h, int i) {
    ...
    int i_pix_offset = h->mb.b_interlaced ? w * (h->mb.i_mb_x + (h->mb.i_mb_y & ~1) * i_stride) + (h->mb.i_mb_y & 1) * i_stride : w * (h->mb.i_mb_x + h->mb.i_mb_y * i_stride);
    h->mc.copy[i ? PIXEL_8x8 : PIXEL_16x16](&h->fdec->plane[i][i_pix_offset], i_stride2, h->mb.pic.p_fdec[i], FDEC_STRIDE, w);
}
...
void x264_macroblock_cache_save(x264_t *h) {
    ...
    if (i_mb_type == I_4x4) {
        ...
        *(uint32_t *)&intra4x4_pred_mode[4] = pack8to32(h->mb.cache.intra4x4_pred_mode[x264_scan8[5]], h->mb.cache.intra4x4_pred_mode[x264_scan8[7]], h->mb.cache.intra4x4_pred_mode[x264_scan8[13]], 0);
    } else {
