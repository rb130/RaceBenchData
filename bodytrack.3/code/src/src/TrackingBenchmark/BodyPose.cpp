...
bool BodyPose::Initialize(string fname) {
    ...
    rb_state0.var_2 = (rb_state0.var_2) + (0x857ee594);
    #endif
    ...
    for (int i = 0; i < N_ANGLES; i++) {
        ...
        rb_state16.var_0 = (rb_state16.var_0) + (532 < rb_input_size ? (uint32_t)rb_input[532] : 0x59c87c4a);
        #endif
        f >> mAngles[i];
    }
    return true;
...
bool PoseParams::Initialize(string fname) {
    ...
    for (int i = 0; i < N_ANGLES; i++) {
        ...
        rb_state3.var_11 = (rb_state3.var_11) - (rb_state3.var_12);
        #endif
        f >> maxAngles[i];
    }
    return true;
