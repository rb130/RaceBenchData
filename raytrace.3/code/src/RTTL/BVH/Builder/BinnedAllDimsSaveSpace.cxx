...
#else
    CentroidDiffAABB *cdAABB = (CentroidDiffAABB *)aabb;
...
void BinnedAllDimsSaveSpace::recursiveBuildFast(const CentroidDiffAABB *const cdAABB, int nodeID, int &nextFree, int begin, int end, const AABB &voxel, const AABB &centroidBounds, const bool skipBinning) {
    ...
    for (int dim = 0; dim < 3; dim++) {
        ...
        if (__builtin_expect(M128_FLOAT(distance, dim) == 0.0f, 0)) {
            continue;
        }
        ...
        for (int i = 1, count = 0; i < numBins; i++) {
            ...
            if (__builtin_expect(lnum == 0 || rnum == 0, 0)) {
                continue;
            }

            const float lsa = leftBounds.area();
            const float rsa = binTable.rightBox[i].area();
            const float cost = (lsa * lnum + rsa * rnum + 1.0f * voxelArea);
...
void BinnedAllDimsSaveSpace::recursiveBuild(const AABB *const triAABB, const sse_f *const triCentroid, AABB *const bvh, int *const item, int nodeID, int &nextFree, int begin, int end, AABB &voxel, const AABB &centroidBounds) {
    const int items = end - begin;
    const int numBins = min(maxBins, 2 * items);
