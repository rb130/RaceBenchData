...
bool verify_ptest(fluid_t *fluid, fluid_t *rfluid, conf_t *conf) {
    ...
    for (i = 0; i < fluid->numParticles; i++) {
        presult = (fluid->p[i].x >= rfluid->p[i].x - conf->ptest.tol) && (fluid->p[i].y >= rfluid->p[i].y - conf->ptest.tol) && (fluid->p[i].z >= rfluid->p[i].z - conf->ptest.tol) && (fluid->p[i].x <= rfluid->p[i].x + conf->ptest.tol) && (fluid->p[i].y <= rfluid->p[i].y + conf->ptest.tol) && (fluid->p[i].z <= rfluid->p[i].z + conf->ptest.tol);
...
bool verify_vtest(fluid_t *fluid, fluid_t *rfluid, conf_t *conf) {
    ...
    for (i = 0; i < fluid->numParticles; i++) {
        presult = (fluid->v[i].x >= rfluid->v[i].x - conf->vtest.tol) && (fluid->v[i].y >= rfluid->v[i].y - conf->vtest.tol) && (fluid->v[i].z >= rfluid->v[i].z - conf->vtest.tol) && (fluid->v[i].x <= rfluid->v[i].x + conf->vtest.tol) && (fluid->v[i].y <= rfluid->v[i].y + conf->vtest.tol) && (fluid->v[i].z <= rfluid->v[i].z + conf->vtest.tol);
        if (!presult && conf->output.verbose && (ecount < conf->output.max)) {
...
bool verify_bbox(fluid_t *fluid, fluid_t *rfluid, conf_t *conf) {
    ...
    result = (fluid->bbox.min.x >= rfluid->bbox.min.x - conf->bbox.tol) && (fluid->bbox.min.y >= rfluid->bbox.min.y - conf->bbox.tol) && (fluid->bbox.min.z >= rfluid->bbox.min.z - conf->bbox.tol) && (fluid->bbox.min.x <= rfluid->bbox.min.x + conf->bbox.tol) && (fluid->bbox.min.y <= rfluid->bbox.min.y + conf->bbox.tol) && (fluid->bbox.min.z <= rfluid->bbox.min.z + conf->bbox.tol) && (fluid->bbox.max.x >= rfluid->bbox.max.x - conf->bbox.tol) && (fluid->bbox.max.y >= rfluid->bbox.max.y - conf->bbox.tol) && (fluid->bbox.max.z >= rfluid->bbox.max.z - conf->bbox.tol) && (fluid->bbox.max.x <= rfluid->bbox.max.x + conf->bbox.tol) && (fluid->bbox.max.y <= rfluid->bbox.max.y + conf->bbox.tol) && (fluid->bbox.max.z <= rfluid->bbox.max.z + conf->bbox.tol);
...
int main(int argc, char *argv[]) {
    ...
    if (conf.output.verbose) {
        std::cout << "Loading fluid \"" << conf.file << "\"..." << std::endl;
    }
    ReadFile(conf.file, &fluid);
    if (conf.output.verbose) {
        std::cout << "Loading reference fluid \"" << conf.rfile << "\"..." << std::endl;
    }
