...
#ifdef JPEG_SUPPORT
static JPEGPrintQTable(fd, tab) FILE *fd;
u_char tab[64];
{
    int i, j;
    char *sep;
...
static JPEGPrintCTable(fd, tab) FILE *fd;
u_char *tab;
{
    int i, n, count;
    char *sep;
...
void TIFFPrintDirectory(tif, fd, flags) TIFF *tif;
FILE *fd;
long flags;
{
    ...
    if (TIFFFieldSet(tif, FIELD_IMAGEDIMENSIONS)) {
        fprintf(fd, "  Image Width: %lu Image Length: %lu", td->td_imagewidth, td->td_imagelength);
    ...
    if (TIFFFieldSet(tif, FIELD_TILEDIMENSIONS)) {
        fprintf(fd, "  Tile Width: %lu Tile Length: %lu", td->td_tilewidth, td->td_tilelength);
    ...
    if (TIFFFieldSet(tif, FIELD_COLORMAP)) {
        ...
        if (flags & TIFFPRINT_COLORMAP) {
            ...
            for (i = 0; i < n; i++) {
                fprintf(fd, "   %5d: %5u %5u %5u\n", i, td->td_colormap[0][i], td->td_colormap[1][i], td->td_colormap[2][i]);
            }
        } else {
            fprintf(fd, "(present)\n");
        }
    ...
    if (TIFFFieldSet(tif, FIELD_TRANSFERFUNCTION)) {
        ...
        if (flags & TIFFPRINT_CURVES) {
            ...
            n = 1L << td->td_bitspersample;
            for (i = 0; i < n; i++) {
                fprintf(fd, "    %2d: %5u", i, td->td_transferfunction[0][i]);
                for (j = 1; j < td->td_samplesperpixel; j++) {
                    fprintf(fd, " %5u", td->td_transferfunction[j][i]);
                }
                putc('\n', fd);
            }
        } else {
