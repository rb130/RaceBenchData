...
Compute_Observer_Transformed_Highlight_Vector() {
    ...
    obseye[X] = obseye[X] * inv_magnitude; racebench_bug65(1); racebench_bug76(1);
    ...
    inv_magnitude = 1.0 / sqrt(obshighlight[X] * obshighlight[X] + obshighlight[Y] * obshighlight[Y] + obshighlight[Z] * obshighlight[Z]); racebench_bug11(1);
