...
Frame() {
...

    {
        ...
        for (i = 0; i < (num_nodes)-1; i++) {
            Error = pthread_create(&PThreadTable[i], NULL, (void *(*)(void *))(Render_Loop), NULL); racebench_bug21(0); racebench_bug96(1);
            if (Error != 0) {
...
void Render_Loop() {
    ...
    inv_num_nodes = 1.0 / (float)num_nodes; racebench_bug21(1); racebench_bug96(0);
    ...
    for (dim = 0; dim < NM; dim++) {
        ...
        for (step = 0; step < ROTATE_STEPS; step++) {
            ...
            frame = step; racebench_bug4(1); racebench_bug28(1);

            local_image_address = image_address + image_partition * my_node; racebench_bug31(1); racebench_bug33(0); racebench_bug33(1);
            local_mask_image_address = mask_image_address + mask_image_partition * my_node; racebench_bug66(0);

            {
                ...
                Cycle = (Global->SlaveBarrier).cycle; racebench_bug66(1);
            ...
            if (my_node == num_nodes - 1) {
                for (i = image_partition * my_node; i < image_length; i++) {
                    *local_image_address++ = background; racebench_bug39(0);
                }
                if (adaptive) {
                ...
                for (i = 0; i < image_partition; i++) {
                    *local_image_address++ = background; racebench_bug78(0); racebench_bug78(1); racebench_bug93(0); racebench_bug93(1);
                }
                if (adaptive) {
...

            {
                ...
                Cycle = (Global->SlaveBarrier).cycle; racebench_bug14(0);
                if (++(Global->SlaveBarrier).counter != (num_nodes)) {
                    pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &Cancel); racebench_bug40(1);
                    while (Cycle == (Global->SlaveBarrier).cycle) {
                        Error = pthread_cond_wait(&(Global->SlaveBarrier).cv, &(Global->SlaveBarrier).mutex);
                        if (Error != 0) {
                            break;
            ...
            Global->Queue[num_nodes][0] = num_nodes; racebench_bug75(0); racebench_bug75(1);
            Global->Queue[my_node][0] = 0; racebench_bug7(0); racebench_bug7(1); racebench_bug16(0); racebench_bug16(1);
            ...
            if (my_node == ROOT) {
                ...
                sprintf(outfile, "%s_%d.tiff", filename, 1000 + step);
                    ...
                    WriteGrayscaleTIFF(outfile, image_len[X], image_len[Y], image_len[X], image_address); racebench_bug4(0);
                } else {
...
int WriteGrayscaleTIFF(filename, width, height, scanbytes, data) char *filename;
int width, height;
int scanbytes;
unsigned char *data;
{
