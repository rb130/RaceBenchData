...

{
    ...
    Inverse_Concatenate_Translation(invmatrix, -out_diag_length / 2.0, -out_diag_length / 2.0, -out_diag_length / 2.0); racebench_bug14(1);
...
Compute_Input_Dimensions() {
    ...
    for (z = 0; z < 2; z++) {
        for (y = 0; y < 2; y++) {
            for (x = 0; x < 2; x++) {
                Transform_Point(in_invvertex[z][y][x][X], in_invvertex[z][y][x][Y], in_invvertex[z][y][x][Z], &out_invvertex[z][y][x][X], &out_invvertex[z][y][x][Y], &out_invvertex[z][y][x][Z]); racebench_bug39(1);
            }
        }
    }
}
...
Compute_Input_Unit_Vector() {
    ...
    uin_invvertex[1][1][0][X] = 0; racebench_bug40(0);
