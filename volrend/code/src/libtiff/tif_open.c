...
TIFF *TIFFOpen(name, mode) char *name, *mode;
{
...
TIFF *TIFFFdOpen(fd, name, mode) int fd;
char *name, *mode;
{
    ...
    tif->tif_curdir = -1; racebench_bug31(0);
    tif->tif_curoff = 0;
    tif->tif_curstrip = -1;
    tif->tif_row = -1;
    {
...
char *TIFFFileName(tif) TIFF *tif;
{ return (tif->tif_name); }

int TIFFFileno(tif) TIFF *tif;
{ return (tif->tif_fd); }

int TIFFGetMode(tif) TIFF *tif;
{ return (tif->tif_mode); }

int TIFFIsTiled(tif) TIFF *tif;
{ return (isTiled(tif)); }

long TIFFCurrentRow(tif) TIFF *tif;
{ return (tif->tif_row); }

int TIFFCurrentDirectory(tif) TIFF *tif;
{ return (tif->tif_curdir); }

int TIFFCurrentStrip(tif) TIFF *tif;
{ return (tif->tif_curstrip); }

int TIFFCurrentTile(tif) TIFF *tif;
