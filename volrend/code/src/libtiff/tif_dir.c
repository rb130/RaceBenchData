...
static DECLARE2(setString, char **, cpp, char *, cp) {
    if (*cpp) {
        free(*cpp), *cpp = 0;
    }
    if (cp) {
...
static DECLARE3(setShortArray, u_short **, wpp, u_short *, wp, long, n) {
    if (*wpp) {
        free((char *)*wpp), *wpp = 0;
    }
    n *= sizeof(u_short);
...
static DECLARE3(setLongArray, u_long **, wpp, u_long *, wp, long, n) {
    if (*wpp) {
        free((char *)*wpp), *wpp = 0;
    }
    n *= sizeof(u_long);
...
static DECLARE3(setFloatArray, float **, wpp, float *, wp, long, n) {
    if (*wpp) {
        free((char *)*wpp), *wpp = 0;
    }
    n *= sizeof(float);
...
static DECLARE3(setJPEGQTable, u_char ***, wpp, u_char **, wp, int, nc) {
    static u_char zigzag[64] = {0, 1, 5, 6, 14, 15, 27, 28, 2, 4, 7, 13, 16, 26, 29, 42, 3, 8, 12, 17, 25, 30, 41, 43, 9, 11, 18, 24, 31, 40, 44, 53, 10, 19, 23, 32, 39, 45, 52, 54, 20, 22, 33, 38, 46, 51, 55, 60, 21, 34, 37, 47, 50, 56, 59, 61, 35, 36, 48, 49, 57, 58, 62, 63};
    char *tab;
    int i, j;
    ...
    *wpp = (u_char **)malloc(nc * (sizeof(u_char *) + 64 * sizeof(u_char)));
...
static DECLARE3(setJPEGCTable, u_char ***, cpp, u_char **, cp, int, nc) {
    u_char *tab;
    int i, j, nw;
...
static TIFFSetField1(tif, tag, ap) TIFF *tif;
int tag;
va_list ap;
{
    ...
    case TIFFTAG_ORIENTATION:
        ...
        if (v < ORIENTATION_TOPLEFT || ORIENTATION_LEFTBOT < v) {
            TIFFWarning(tif->tif_name, "Bad value %ld for \"%s\" tag ignored", v, TIFFFieldWithTag(tag)->field_name);
        } else {
            td->td_orientation = v;
        }
        break;
    ...
    case TIFFTAG_JPEGQTABLES:
        setJPEGQTable(&td->td_qtab, va_arg(ap, u_char **), td->td_samplesperpixel);
        break;
    case TIFFTAG_JPEGDCTABLES:
        setJPEGCTable(&td->td_dctab, va_arg(ap, u_char **), td->td_samplesperpixel);
        break;
    case TIFFTAG_JPEGACTABLES:
        setJPEGCTable(&td->td_actab, va_arg(ap, u_char **), td->td_samplesperpixel);
        break;
#endif
    ...
    case TIFFTAG_TRANSFERFUNCTION:
        for (i = 0; i < td->td_samplesperpixel; i++) {
            setShortArray(&td->td_transferfunction[i], va_arg(ap, u_short *), 1L << td->td_bitspersample);
        }
        break;
...
#endif
    default:
        TIFFError(tif->tif_name, "Internal error, tag value botch, tag \"%s\"", TIFFFieldWithTag(tag)->field_name);
        status = 0;
        break;
    }
    if (status) {
    ...
    return (status);
badvalue:
    TIFFError(tif->tif_name, "%ld: Bad value for \"%s\"", v, TIFFFieldWithTag(tag)->field_name);
    va_end(ap);
    return (0);
}

static OkToChangeTag(tif, tag) TIFF *tif;
int tag;
{
...
TIFFVSetField(tif, tag, ap) TIFF *tif;
int tag;
va_list ap;
{
    int status = 0;
    ...
    if (!OkToChangeTag(tif, tag)) {
        TIFFFieldInfo const *fip = TIFFFindFieldInfo(tag, TIFF_ANY);
        if (fip) {
            TIFFError("TIFFVSetField", "%s: Cannot modify tag \"%s\" while writing", tif->tif_name, fip->field_name);
        }
    } else {
        status = TIFFSetField1(tif, tag, ap);
    }
...
static TIFFGetField1(td, tag, ap) TIFFDirectory *td;
int tag;
va_list ap;
{
    ...
    case TIFFTAG_TILEDEPTH:
        *va_arg(ap, u_long *) = td->td_tiledepth;
        break;
    case TIFFTAG_DATATYPE:
        *va_arg(ap, u_short *) = (td->td_sampleformat == SAMPLEFORMAT_VOID ? 0 : td->td_sampleformat);
        break;
    case TIFFTAG_SAMPLEFORMAT:
...
#endif
    default:
        TIFFError("TIFFGetField1", "Internal error, no value returned for tag \"%s\"", TIFFFieldWithTag(tag)->field_name);
        break;
    }
    va_end(ap);
}
...
TIFFVGetField(tif, tag, ap) TIFF *tif;
int tag;
va_list ap;
{
    TIFFFieldInfo const *fip = TIFFFindFieldInfo(tag, TIFF_ANY);
...
TIFFFreeDirectory(tif) TIFF *tif;
{
...
TIFFDefaultDirectory(tif) TIFF *tif;
{
    register TIFFDirectory *td = &tif->tif_dir;
...
TIFFSetDirectory(tif, dirn) register TIFF *tif;
int dirn;
{
