...
u_int TIFFComputeTile(tif, x, y, s, z) TIFF *tif;
u_long x, y, z;
u_int s;
{
...
u_int TIFFNumberOfTiles(tif) TIFF *tif;
{
    TIFFDirectory *td = &tif->tif_dir;
    u_long dx = td->td_tilewidth;
    u_long dy = td->td_tilelength;
...
u_long TIFFTileRowSize(tif) TIFF *tif;
{
    TIFFDirectory *td = &tif->tif_dir;
    u_long rowsize;
...
u_long TIFFVTileSize(tif, nrows) TIFF *tif;
u_long nrows;
{
    TIFFDirectory *td = &tif->tif_dir;
    u_long tilesize;
...
u_long TIFFTileSize(tif) TIFF *tif;
