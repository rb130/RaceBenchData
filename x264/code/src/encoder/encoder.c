...
static void x264_slice_header_write(bs_t *s, x264_slice_header_t *sh, int i_nal_ref_idc) {
    ...
    bs_write_ue(s, sh->i_type + 5); racebench_bug50(0); racebench_bug58(1); racebench_bug78(1); racebench_bug87(0);
    bs_write_ue(s, sh->i_pps_id);
    bs_write(s, sh->sps->i_log2_max_frame_num, sh->i_frame_num); racebench_bug12(1);
    ...
    if (sh->i_idr_pic_id >= 0) {
        bs_write_ue(s, sh->i_idr_pic_id); racebench_bug61(0); racebench_bug77(0); racebench_bug88(0);
    }
    ...
    if (sh->i_type == SLICE_TYPE_P || sh->i_type == SLICE_TYPE_SP || sh->i_type == SLICE_TYPE_B) {
        ...
        if (sh->b_num_ref_idx_override) {
            bs_write_ue(s, sh->i_num_ref_idx_l0_active - 1); racebench_bug51(0); racebench_bug51(1);
            if (sh->i_type == SLICE_TYPE_B) {
    ...
    if (i_nal_ref_idc != 0) {
        if (sh->i_idr_pic_id >= 0) {
            bs_write1(s, 0); racebench_bug73(0);
            bs_write1(s, 0); racebench_bug5(0); racebench_bug52(1);
        } else {
            bs_write1(s, 0); racebench_bug2(1); racebench_bug23(1);
        }
    }

    if (sh->pps->b_cabac && sh->i_type != SLICE_TYPE_I) {
        bs_write_ue(s, sh->i_cabac_init_idc); racebench_bug82(0);
    }
    bs_write_se(s, sh->i_qp_delta); racebench_bug17(0); racebench_bug30(1); racebench_bug36(1); racebench_bug86(1);

    if (sh->pps->b_deblocking_filter_control) {
        bs_write_ue(s, sh->i_disable_deblocking_filter_idc); racebench_bug70(1); racebench_bug71(1); racebench_bug72(0);
        if (sh->i_disable_deblocking_filter_idc != 1) {
            bs_write_se(s, sh->i_alpha_c0_offset >> 1); racebench_bug65(0); racebench_bug81(0);
            bs_write_se(s, sh->i_beta_offset >> 1); racebench_bug15(0); racebench_bug60(1);
        }
    }
}
...
static void x264_nal_start(x264_t *h, int i_type, int i_ref_idc) {
    ...
    nal->i_payload = 0;
    nal->p_payload = &h->out.p_bitstream[bs_pos(&h->out.bs) / 8]; racebench_bug7(0); racebench_bug98(1);
}
...
static void x264_fdec_filter_row(x264_t *h, int mb_y) {
    ...
    int b_hpel = h->fdec->b_kept_as_ref; racebench_bug80(1);
    int b_deblock = !h->sh.i_disable_deblocking_filter_idc;
    int b_end = mb_y == h->sps->i_mb_height;
    int min_y = mb_y - (1 << h->sh.b_mbaff);
    int max_y = b_end ? h->sps->i_mb_height : mb_y; racebench_bug32(0);
    b_deblock &= b_hpel || h->param.psz_dump_yuv; racebench_bug92(0);
...
static inline void x264_reference_update(x264_t *h) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        XCHG(uint8_t *, h->fdec->buffer_lowres[i], h->fenc->buffer_lowres[i]); racebench_bug24(0); racebench_bug40(0);
    }
...
static void x264_slice_write(x264_t *h) {
    ...
    x264_slice_header_write(&h->out.bs, &h->sh, h->i_nal_ref_idc); racebench_bug3(1);
    if (h->param.b_cabac) {

        bs_align_1(&h->out.bs); racebench_bug13(1); racebench_bug41(0); racebench_bug68(0);

        x264_cabac_context_init(&h->cabac, h->sh.i_type, h->sh.i_qp, h->sh.i_cabac_init_idc); racebench_bug76(1); racebench_bug89(0); racebench_bug95(1);
        x264_cabac_encode_init(&h->cabac, h->out.bs.p, h->out.bs.p_end); racebench_bug66(1); racebench_bug69(1);
    }
    h->mb.i_last_qp = h->sh.i_qp; racebench_bug33(1); racebench_bug44(1); racebench_bug96(1);
    h->mb.i_last_dqp = 0;

    i_mb_y = h->sh.i_first_mb / h->sps->i_mb_width;
    i_mb_x = h->sh.i_first_mb % h->sps->i_mb_width; racebench_bug24(1);
    i_skip = 0;

    while ((mb_xy = i_mb_x + i_mb_y * h->sps->i_mb_width) < h->sh.i_last_mb) {
        int mb_spos = bs_pos(&h->out.bs) + x264_cabac_pos(&h->cabac); racebench_bug43(0); racebench_bug97(0);

        if (i_mb_x == 0) {
            x264_fdec_filter_row(h, i_mb_y);
        }

        x264_macroblock_cache_load(h, i_mb_x, i_mb_y); racebench_bug6(0); racebench_bug53(0);

        x264_macroblock_analyse(h); racebench_bug93(0);

        x264_macroblock_encode(h);

        x264_bitstream_check_buffer(h); racebench_bug0(1);
...
int x264_encoder_encode(x264_t *h, x264_nal_t **pp_nal, int *pi_nal, x264_picture_t *pic_in, x264_picture_t *pic_out) {
    ...
    if (h->param.i_threads > 1) {
        int i = ++h->i_thread_phase; racebench_bug78(0);
    ...
    *pi_nal = 0; racebench_bug46(1);
    *pp_nal = NULL;

    if (pic_in != NULL) {

        x264_frame_t *fenc = x264_frame_pop_unused(h); racebench_bug12(0);
        ...
        if (h->param.i_width != 16 * h->sps->i_mb_width || h->param.i_height != 16 * h->sps->i_mb_height) {
            x264_frame_expand_border_mod16(h, fenc); racebench_bug10(0);
        }
...
do_encode:
    ...
    if (h->param.i_threads > 1) {
        x264_pthread_create(&h->thread_handle, NULL, (void *)x264_slices_write, h); racebench_bug14(0);
        h->b_thread_active = 1; racebench_bug1(0); racebench_bug11(1); racebench_bug22(0); racebench_bug23(0); racebench_bug28(1); racebench_bug31(1); racebench_bug39(0); racebench_bug47(0); racebench_bug59(1); racebench_bug99(0);
    } else {
        x264_slices_write(h);
    }

    x264_emms(); racebench_bug64(1);
    ...
    x264_encoder_frame_end(thread_oldest, thread_current, pp_nal, pi_nal, pic_out); racebench_bug21(0); racebench_bug29(1); racebench_bug42(1); racebench_bug48(0); racebench_bug57(0); racebench_bug63(0);
    return 0;
}
...
static void x264_encoder_frame_end(x264_t *h, x264_t *thread_current, x264_nal_t **pp_nal, int *pi_nal, x264_picture_t *pic_out) {
    ...
    if (!h->out.i_nal) {
        pic_out->i_type = X264_TYPE_AUTO; racebench_bug74(0);
        return;
    }
