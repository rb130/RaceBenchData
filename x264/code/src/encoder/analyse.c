...
static void x264_mb_analyse_load_costs(x264_t *h, x264_mb_analysis_t *a) {
    ...
    if (!p_cost_mv[a->i_qp]) {
        ...
        p_cost_mv[a->i_qp] += 2 * 4 * 2048; racebench_bug16(0);
        for (i = 0; i <= 2 * 4 * 2048; i++) {
            p_cost_mv[a->i_qp][-i] = p_cost_mv[a->i_qp][i] = a->i_lambda * bs_size_se(i); racebench_bug4(1); racebench_bug5(1); racebench_bug17(1); racebench_bug18(1); racebench_bug20(1); racebench_bug27(1); racebench_bug30(0); racebench_bug33(0); racebench_bug34(1); racebench_bug35(0); racebench_bug36(0); racebench_bug41(1); racebench_bug50(1); racebench_bug52(0); racebench_bug53(1); racebench_bug56(1); racebench_bug58(0); racebench_bug60(0); racebench_bug61(1); racebench_bug65(1); racebench_bug68(1); racebench_bug70(0); racebench_bug71(0); racebench_bug76(0); racebench_bug77(1); racebench_bug79(0); racebench_bug80(0); racebench_bug81(1); racebench_bug84(0); racebench_bug85(1); racebench_bug87(1); racebench_bug88(1); racebench_bug89(1); racebench_bug90(0); racebench_bug97(1); racebench_bug98(0);
        }
    }
...
static void x264_mb_analyse_init(x264_t *h, x264_mb_analysis_t *a, int i_qp) {
    ...
    h->mb.i_subpel_refine = h->param.analyse.i_subpel_refine; racebench_bug28(0);
    h->mb.b_chroma_me = h->param.analyse.b_chroma_me && h->sh.i_type == SLICE_TYPE_P && h->mb.i_subpel_refine >= 5;
    h->mb.b_trellis = h->param.analyse.i_trellis > 1 && a->i_mbrd; racebench_bug11(0);
...
#define CLIP_FMV(mv) x264_clip3(mv, -i_fmv_range, i_fmv_range - 1)
        h->mb.mv_min[0] = 4 * (-16 * h->mb.i_mb_x - 24); racebench_bug59(0);
        h->mb.mv_max[0] = 4 * (16 * (h->sps->i_mb_width - h->mb.i_mb_x - 1) + 24); racebench_bug9(0);
        h->mb.mv_min_spel[0] = CLIP_FMV(h->mb.mv_min[0]);
        h->mb.mv_max_spel[0] = CLIP_FMV(h->mb.mv_max[0]);
        h->mb.mv_min_fpel[0] = (h->mb.mv_min_spel[0] >> 2) + i_fpel_border; racebench_bug25(0);
        ...
        if (h->mb.i_mb_x == 0) {
            ...
            if (h->param.i_threads > 1) {
                ...
                int thresh = pix_y + h->param.analyse.i_mv_range_thread; racebench_bug49(0);
...
static inline void x264_mb_cache_fenc_satd(x264_t *h) {
    ...
    for (y = 0; y < 4; y++) {
        for (x = 0; x < 4; x++) {
            fenc = h->mb.pic.p_fenc[0] + x * 4 + y * 4 * FENC_STRIDE; racebench_bug55(1);
    ...
    for (y = 0; y < 2; y++) {
        for (x = 0; x < 2; x++) {
            fenc = h->mb.pic.p_fenc[0] + x * 8 + y * 8 * FENC_STRIDE; racebench_bug79(1);
...
void x264_macroblock_analyse(x264_t *h) {
    ...
    h->mb.i_qp = x264_ratecontrol_qp(h); racebench_bug34(0);
