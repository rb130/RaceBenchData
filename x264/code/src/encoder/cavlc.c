...
static const uint8_t intra4x4_cbp_to_golomb[48] = {3, 29, 30, 17, 31, 18, 37, 8, 32, 38, 19, 9, 20, 10, 11, 2, 16, 33, 34, 21, 35, 22, 39, 4, 36, 40, 23, 5, 24, 6, 7, 1, 41, 42, 43, 25, 44, 26, 46, 12, 45, 47, 27, 13, 28, 14, 15, 0};
static const uint8_t inter_cbp_to_golomb[48] = {0, 2, 3, 7, 4, 8, 17, 13, 5, 18, 9, 14, 10, 15, 16, 11, 1, 32, 33, 36, 34, 37, 44, 40, 35, 45, 38, 41, 39, 42, 43, 19, 6, 24, 25, 20, 26, 21, 46, 28, 27, 47, 22, 29, 23, 30, 31, 12};
static const uint8_t mb_type_b_to_golomb[3][9] = {{4, 8, 12, 10, 6, 14, 16, 18, 20}, {5, 9, 13, 11, 7, 15, 17, 19, 21}, {1, -1, -1, -1, 2, -1, -1, -1, 3}};
static const uint8_t sub_mb_type_p_to_golomb[4] = {3, 1, 2, 0};
static const uint8_t sub_mb_type_b_to_golomb[13] = {10, 4, 5, 1, 11, 6, 7, 2, 12, 8, 9, 3, 0};
...
static void block_residual_write_cavlc(x264_t *h, bs_t *s, int i_ctxBlockCat, int i_idx, int16_t *l, int i_count) {
    ...
    for (i = i_trailing; i < i_total; i++) {
        ...
        if ((i_level_code >> i_suffix_length) < 14) {
            bs_write(s, (i_level_code >> i_suffix_length) + 1 + i_suffix_length, (1 << i_suffix_length) + (i_level_code & ((1 << i_suffix_length) - 1)));
            ...
            bs_write(s, 15 + i_suffix_length, (1 << i_suffix_length) + (i_level_code & ((1 << i_suffix_length) - 1)));
        } else {
...
static void cavlc_qp_delta(x264_t *h, bs_t *s) {
    ...
    if (h->mb.i_type == I_16x16 && !(h->mb.i_cbp_luma | h->mb.i_cbp_chroma) && !h->mb.cache.non_zero_count[x264_scan8[24]]) {
#if !RDO_SKIP_BS
...
void x264_macroblock_write_cavlc(x264_t *h, bs_t *s) {
    ...
    if (i_mb_type == I_4x4 || i_mb_type == I_8x8) {
        ...
        bs_write_ue(s, i_mb_i_offset + 1 + x264_mb_pred_mode16x16_fix[h->mb.i_intra16x16_pred_mode] + h->mb.i_cbp_chroma * 4 + (h->mb.i_cbp_luma == 0 ? 0 : 12));
        bs_write_ue(s, x264_mb_pred_mode8x8c_fix[h->mb.i_chroma_pred_mode]);
        ...
        int b_sub_ref0;
        if ((h->mb.cache.ref[0][x264_scan8[0]] | h->mb.cache.ref[0][x264_scan8[4]] | h->mb.cache.ref[0][x264_scan8[8]] | h->mb.cache.ref[0][x264_scan8[12]]) == 0) {
            bs_write_ue(s, 4);
            b_sub_ref0 = 0;
        } else {
            bs_write_ue(s, 3);
        ...
        for (i_list = 0; i_list < 2; i_list++) {
            ...
            if (i_ref_max) {
                ...
                case D_16x16:
                    if (b_list[i_list][0]) {
                        bs_write_te(s, i_ref_max, h->mb.cache.ref[i_list][x264_scan8[0]]);
                    }
                    break;
                case D_16x8:
                    if (b_list[i_list][0]) {
                    ...
                    if (b_list[i_list][1]) {
                        bs_write_te(s, i_ref_max, h->mb.cache.ref[i_list][x264_scan8[8]]);
                    }
                    break;
                case D_8x16:
                    if (b_list[i_list][0]) {
                    ...
                    if (b_list[i_list][1]) {
                        bs_write_te(s, i_ref_max, h->mb.cache.ref[i_list][x264_scan8[4]]);
                    }
                    break;
                }
            }
