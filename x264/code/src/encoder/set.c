...
static void scaling_list_write(bs_t *s, x264_pps_t *pps, int idx) {
    ...
    const uint8_t *def_list = (idx == CQM_4IC) ? pps->scaling_list[CQM_4IY] : (idx == CQM_4PC) ? pps->scaling_list[CQM_4PY] : x264_cqm_jvt[idx];
    if (!memcmp(list, def_list, len)) {
        ...
        for (j = 0; j < run; j++) {
            bs_write_se(s, (int8_t)(list[zigzag[j]] - (j > 0 ? list[zigzag[j - 1]] : 8)));
        }
...
void x264_sps_init(x264_sps_t *sps, int i_id, x264_param_t *param) {
    ...
    sps->i_poc_type = 0;
    if (sps->i_poc_type == 0) {
        sps->i_log2_max_poc_lsb = sps->i_log2_max_frame_num + 1;
    ...
    sps->b_direct8x8_inference = param->analyse.i_direct_8x8_inference || !sps->b_frame_mbs_only || !(param->analyse.inter & X264_ANALYSE_PSUB8x8);
    ...
    sps->vui.i_max_dec_frame_buffering = sps->i_num_ref_frames = X264_MIN(16, X264_MAX(param->i_frame_reference, 1 + sps->vui.i_num_reorder_frames));
    ...
    if (sps->b_vui) {
        ...
        if (sps->vui.b_aspect_ratio_info_present) {
            ...
            static const struct {
                int w, h;
                int sar;
            } sar[] = {{1, 1, 1}, {12, 11, 2}, {10, 11, 3}, {16, 11, 4}, {40, 33, 5}, {24, 11, 6}, {20, 11, 7}, {32, 11, 8}, {80, 33, 9}, {18, 11, 10}, {15, 11, 11}, {64, 33, 12}, {160, 99, 13}, {0, 0, -1}};
...
void x264_sei_version_write(x264_t *h, bs_t *s) {
    ...
    const uint8_t uuid[16] = {0xdc, 0x45, 0xe9, 0xbd, 0xe6, 0xd9, 0x48, 0xb7, 0x96, 0x2c, 0xd8, 0x20, 0xd9, 0x23, 0xee, 0xef};
    char *opts = x264_param2string(&h->param, 0);
    char *version = x264_malloc(200 + strlen(opts));
    int length;

    sprintf(version,
        "x264 - core %d%s - H.264/MPEG-4 AVC codec - "
...
const x264_level_t x264_levels[] = {{10, 1485, 99, 152064, 64, 175, 64, 64, 0, 0, 0, 1},

    {11, 3000, 396, 345600, 192, 500, 128, 64, 0, 0, 0, 1}, {12, 6000, 396, 912384, 384, 1000, 128, 64, 0, 0, 0, 1}, {13, 11880, 396, 912384, 768, 2000, 128, 64, 0, 0, 0, 1}, {20, 11880, 396, 912384, 2000, 2000, 128, 64, 0, 0, 0, 1}, {21, 19800, 792, 1824768, 4000, 4000, 256, 64, 0, 0, 0, 0}, {22, 20250, 1620, 3110400, 4000, 4000, 256, 64, 0, 0, 0, 0}, {30, 40500, 1620, 3110400, 10000, 10000, 256, 32, 22, 0, 1, 0}, {31, 108000, 3600, 6912000, 14000, 14000, 512, 16, 60, 1, 1, 0}, {32, 216000, 5120, 7864320, 20000, 20000, 512, 16, 60, 1, 1, 0}, {40, 245760, 8192, 12582912, 20000, 25000, 512, 16, 60, 1, 1, 0}, {41, 245760, 8192, 12582912, 50000, 62500, 512, 16, 24, 1, 1, 0}, {42, 522240, 8704, 13369344, 50000, 62500, 512, 16, 24, 1, 1, 1}, {50, 589824, 22080, 42393600, 135000, 135000, 512, 16, 24, 1, 1, 1}, {51, 983040, 36864, 70778880, 240000, 240000, 512, 16, 24, 1, 1, 1}, {0}};
