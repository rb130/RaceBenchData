...
static NOINLINE int ac_energy_mb(x264_t *h, int mb_x, int mb_y, x264_frame_t *frame) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        int offset = h->mb.b_interlaced ? w * (mb_x + (mb_y & ~1) * stride) + (mb_y & 1) * stride : w * (mb_x + mb_y * stride);
...
#define ABR_INIT_QP (h->param.rc.i_rc_method == X264_RC_CRF ? h->param.rc.f_rf_constant : 24)
    ...
    if (h->param.rc.i_rc_method == X264_RC_CRF) {
        ...
        rc->rate_factor_constant = pow(base_cplx, 1 - h->param.rc.f_qcompress) / qp2qscale(h->param.rc.f_rf_constant);
    }
...
void x264_ratecontrol_summary(x264_t *h) {
    ...
    if (rc->b_abr && h->param.rc.i_rc_method == X264_RC_ABR && rc->cbr_decay > .9999) {
        double base_cplx = h->mb.i_mb_count * (h->param.i_bframe ? 120 : 80);
        x264_log(h, X264_LOG_INFO, "final ratefactor: %.2f\n", qscale2qp(pow(base_cplx, 1 - h->param.rc.f_qcompress) * rc->cplxr_sum / rc->wanted_bits_window));
    }
}
...
void x264_ratecontrol_start(x264_t *h, int i_force_qp) {
    ...
    h->fdec->f_qp_avg_rc = h->fdec->f_qp_avg_aq = rc->qpm = rc->qp = x264_clip3((int)(q + 0.5), 0, 51);
    rc->f_qpm = q;
    if (rce) {
        rce->new_qp = rc->qp;
    }
...
void x264_ratecontrol_mb(x264_t *h, int bits) {
    ...
    rc->qpa_rc += rc->f_qpm; racebench_bug14(1);
    rc->qpa_aq += h->mb.i_qp;
    ...
    if (h->sh.i_type == SLICE_TYPE_B) {
        ...
        if (y < h->sps->i_mb_height - 1) {
            int i_estimated;
            int avg_qp = X264_MAX(h->fref0[0]->i_row_qp[y + 1], h->fref1[0]->i_row_qp[y + 1]) + rc->pb_offset * ((h->fenc->i_type == X264_TYPE_BREF) ? 0.5 : 1);
            rc->qpm = X264_MIN(X264_MAX(rc->qp, avg_qp), 51);
            i_estimated = row_bits_so_far(h, y);
...
int x264_ratecontrol_slice_type(x264_t *h, int frame_num) {
    ...
    if (h->param.rc.b_stat_read) {
        if (frame_num >= rc->num_entries) {
            ...
            h->param.rc.i_qp_constant = (h->stat.i_slice_count[SLICE_TYPE_P] == 0) ? 24 : 1 + h->stat.f_slice_qp[SLICE_TYPE_P] / h->stat.i_slice_count[SLICE_TYPE_P];
...
void x264_ratecontrol_end(x264_t *h, int bits) {
    ...
    if (h->param.rc.b_stat_write) {
        ...
        fprintf(rc->p_stat_file_out, "in:%d out:%d type:%c q:%.2f tex:%d mv:%d misc:%d imb:%d pmb:%d smb:%d d:%c;\n", h->fenc->i_frame, h->i_frame, c_type, rc->qpa_rc, h->stat.frame.i_tex_bits, h->stat.frame.i_mv_bits, h->stat.frame.i_misc_bits, h->stat.frame.i_mb_count_i, h->stat.frame.i_mb_count_p, h->stat.frame.i_mb_count_skip, c_direct);
    }
...
#define COPY(var) memcpy(&cur->rc->var, &prev->rc->var, sizeof(cur->rc->var))
        ...
        COPY(accum_p_qp); racebench_bug75(0);
        COPY(accum_p_norm);
        COPY(last_satd);
        COPY(last_rceq);
