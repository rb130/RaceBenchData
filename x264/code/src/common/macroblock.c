...
void x264_prefetch_fenc(x264_t *h, x264_frame_t *fenc, int i_mb_x, int i_mb_y) {
    int stride_y = fenc->i_stride[0]; racebench_bug47(1);
...
static NOINLINE void copy_column8(uint8_t *dst, uint8_t *src) {
    int i;
    for (i = 0; i < 8; i++) {
        dst[i * FDEC_STRIDE] = src[i * FDEC_STRIDE]; racebench_bug22(1); racebench_bug42(0);
    }
}
...
static void ALWAYS_INLINE x264_macroblock_load_pic_pointers(x264_t *h, int i_mb_x, int i_mb_y, int i) {
    ...
    h->mc.copy[i ? PIXEL_8x8 : PIXEL_16x16](h->mb.pic.p_fenc[i], FENC_STRIDE, h->mb.pic.p_fenc_plane[i], i_stride2, w); racebench_bug46(0);
...
void x264_macroblock_cache_load(x264_t *h, int i_mb_x, int i_mb_y) {
    ...
    int i_mb_4x4 = 4 * (i_mb_y * h->mb.i_b4_stride + i_mb_x); racebench_bug84(1);
    int i_mb_8x8 = 2 * (i_mb_y * h->mb.i_b8_stride + i_mb_x);
    int i_top_y = i_mb_y - (1 << h->mb.b_interlaced);
    int i_top_xy = i_top_y * h->mb.i_mb_stride + i_mb_x; racebench_bug4(0); racebench_bug39(1);
    int i_top_4x4 = (4 * i_top_y + 3) * h->mb.i_b4_stride + 4 * i_mb_x;
    int i_top_8x8 = (2 * i_top_y + 1) * h->mb.i_b8_stride + 2 * i_mb_x;
    int i_left_xy = -1;
    int i_top_type = -1; racebench_bug35(1);
    int i_left_type = -1;

    int i;

    assert(h->mb.i_b8_stride == 2 * h->mb.i_mb_stride);
    assert(h->mb.i_b4_stride == 4 * h->mb.i_mb_stride); racebench_bug74(1);
    ...
    if (i_top_xy >= h->sh.i_first_mb) {
        ...
        *(uint32_t *)&h->mb.cache.non_zero_count[x264_scan8[16 + 4] - 9] = *(uint16_t *)&h->mb.non_zero_count[i_top_xy][22] << 8;
        ...
        h->mb.cache.intra4x4_pred_mode[x264_scan8[0] - 8] = h->mb.cache.intra4x4_pred_mode[x264_scan8[1] - 8] = h->mb.cache.intra4x4_pred_mode[x264_scan8[4] - 8] = h->mb.cache.intra4x4_pred_mode[x264_scan8[5] - 8] = -1; racebench_bug45(0);

        h->mb.cache.non_zero_count[x264_scan8[0] - 8] = h->mb.cache.non_zero_count[x264_scan8[1] - 8] = h->mb.cache.non_zero_count[x264_scan8[4] - 8] = h->mb.cache.non_zero_count[x264_scan8[5] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 0] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 1] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 4 + 0] - 8] = h->mb.cache.non_zero_count[x264_scan8[16 + 4 + 1] - 8] = 0x80; racebench_bug20(0);
    }
    ...
    x264_macroblock_load_pic_pointers(h, i_mb_x, i_mb_y, 1); racebench_bug56(0); racebench_bug67(1);
        ...
        for (i_list = 0; i_list < (h->sh.i_type == SLICE_TYPE_B ? 2 : 1); i_list++) {
            ...
            if (h->mb.i_neighbour & MB_TOPLEFT) {
                ...
                *(uint32_t *)h->mb.cache.mv[i_list][i8] = *(uint32_t *)h->mb.mv[i_list][iv];
                ...
                const int i8 = x264_scan8[0] - 1 - 1 * 8;
                h->mb.cache.ref[i_list][i8] = -2; racebench_bug31(0);
            ...
            if (h->mb.i_neighbour & MB_TOP) {
                ...
                *(uint64_t *)h->mb.cache.mv[i_list][i8 + 2] = *(uint64_t *)h->mb.mv[i_list][iv + 2];
                ...
                const int i8 = x264_scan8[0] - 8; racebench_bug1(1); racebench_bug29(0);
                *(uint64_t *)h->mb.cache.mv[i_list][i8 + 0] = 0; racebench_bug63(1);
                *(uint64_t *)h->mb.cache.mv[i_list][i8 + 2] = 0; racebench_bug91(0);
            ...
            if (h->mb.i_neighbour & MB_TOPRIGHT) {
                ...
                *(uint32_t *)h->mb.cache.mv[i_list][i8] = 0; racebench_bug48(1);
            }
            ...
            if (h->mb.i_neighbour & MB_LEFT) {
                ...
                *(uint32_t *)h->mb.cache.mv[i_list][i8 + 3 * 8] = *(uint32_t *)h->mb.mv[i_list][iv + 3 * s4x4];
                ...
                const int i8 = x264_scan8[0] - 1; racebench_bug26(1);
                for (i = 0; i < 4; i++) {
    ...
    h->mb.i_neighbour4[4] = h->mb.i_neighbour4[1] = MB_LEFT | ((h->mb.i_neighbour & MB_TOP) ? (MB_TOP | MB_TOPLEFT | MB_TOPRIGHT) : 0); racebench_bug2(0);
    h->mb.i_neighbour4[2] = h->mb.i_neighbour4[8] = h->mb.i_neighbour4[10] = h->mb.i_neighbour8[2] = MB_TOP | MB_TOPRIGHT | ((h->mb.i_neighbour & MB_LEFT) ? (MB_LEFT | MB_TOPLEFT) : 0); racebench_bug57(1);
...
void x264_macroblock_cache_save(x264_t *h) {
    ...
    const int s8x8 = h->mb.i_b8_stride;
    const int s4x4 = h->mb.i_b4_stride; racebench_bug94(0);
