...
static const int dequant4_scale[6][3] = {{10, 13, 16}, {11, 14, 18}, {13, 16, 20}, {14, 18, 23}, {16, 20, 25}, {18, 23, 29}};
...
static const int quant8_scan[16] = {0, 3, 4, 3, 3, 1, 5, 1, 4, 5, 2, 5, 3, 1, 5, 1};
...
static const int quant8_scale[6][6] = {{13107, 11428, 20972, 12222, 16777, 15481}, {11916, 10826, 19174, 11058, 14980, 14290}, {10082, 8943, 15978, 9675, 12710, 11985}, {9362, 8228, 14913, 8931, 11984, 11259}, {8192, 7346, 13159, 7740, 10486, 9777}, {7282, 6428, 11570, 6830, 9118, 8640}};
...
int x264_cqm_init(x264_t *h) {
    ...
    int q, i, j, i_list;
    int deadzone[4] = {32 - h->param.analyse.i_luma_deadzone[1], 32 - h->param.analyse.i_luma_deadzone[0], 32 - 11, 32 - 21};
    int max_qp_err = -1;
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = (i < 4 ? 0 : 4); j < i; j++) {
            if (deadzone[j & 3] == deadzone[i & 3] && !memcmp(h->pps->scaling_list[i], h->pps->scaling_list[j], size * sizeof(uint8_t))) {
                break;
            }
        }
        if (j < i) {
...
void x264_cqm_delete(x264_t *h) {
    int i, j;
	void **quant4_mf = h->quant4_mf;
	void **quant4_bias = h->quant4_bias;
    for (i = 0; i < 6; i++) {
        for (j = 0; j < i; j++) {
            if (quant4_mf[i] == quant4_mf[j]) {
                break;
            }
        }
        if (j == i) {
        ...
        for (j = 0; j < i; j++) {
            if (quant4_bias[i] == quant4_bias[j]) {
                break;
            }
        }
        if (j == i) {
...
static int x264_cqm_parse_jmlist(x264_t *h, const char *buf, const char *name, uint8_t *cqm, const uint8_t *jvt, int length) {
    char *p;
    char *nextvar;
    int i;
