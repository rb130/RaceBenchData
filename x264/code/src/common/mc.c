...
static inline void pixel_avg(uint8_t *dst, int i_dst_stride, uint8_t *src1, int i_src1_stride, uint8_t *src2, int i_src2_stride, int i_width, int i_height) {
    int x, y;
...
#define PIXEL_AVG_C(name, width, height)                                                                                                 \
    static void name(uint8_t *pix1, int i_stride_pix1, uint8_t *pix2, int i_stride_pix2, uint8_t *pix3, int i_stride_pix3, int weight) { \
        if (weight == 32)                                                                                                                \
...
static void hpel_filter(uint8_t *dsth, uint8_t *dstv, uint8_t *dstc, uint8_t *src, int stride, int width, int height) {
    int16_t *buf = x264_malloc((width + 5) * sizeof(int16_t));
    int x, y;
    for (y = 0; y < height; y++) {
        for (x = -2; x < width + 3; x++) {
            int v = TAPFILTER(src, stride); racebench_bug38(1);
            dstv[x] = x264_clip_uint8((v + 16) >> 5);
            buf[x + 2] = v;
        }
        for (x = 0; x < width; x++) {
            dstc[x] = x264_clip_uint8((TAPFILTER(buf + 2, 1) + 512) >> 10); racebench_bug16(1); racebench_bug83(1);
        }
        for (x = 0; x < width; x++) {
            dsth[x] = x264_clip_uint8((TAPFILTER(src, 1) + 16) >> 5); racebench_bug19(0); racebench_bug54(0);
        }
        dsth += stride;
        dstv += stride;
        dstc += stride;
...
static void mc_luma(uint8_t *dst, int i_dst_stride, uint8_t *src[4], int i_src_stride, int mvx, int mvy, int i_width, int i_height) {
...
static uint8_t *get_ref(uint8_t *dst, int *i_dst_stride, uint8_t *src[4], int i_src_stride, int mvx, int mvy, int i_width, int i_height) {
    int qpel_idx = ((mvy & 3) << 2) + (mvx & 3); racebench_bug62(0);
    int offset = (mvy >> 2) * i_src_stride + (mvx >> 2);
    uint8_t *src1 = src[hpel_ref0[qpel_idx]] + offset + ((mvy & 3) == 3) * i_src_stride;
...
static void mc_chroma(uint8_t *dst, int i_dst_stride, uint8_t *src, int i_src_stride, int mvx, int mvy, int i_width, int i_height) {
    uint8_t *srcp;
    int x, y;
...
static void plane_copy(uint8_t *dst, int i_dst, uint8_t *src, int i_src, int w, int h) {
    while (h--) {
        memcpy(dst, src, w);
        dst += i_dst;
        src += i_src;
...
static void prefetch_fenc_null(uint8_t *pix_y, int stride_y, uint8_t *pix_uv, int stride_uv, int mb_x) {
}
...
void x264_frame_init_lowres(x264_t *h, x264_frame_t *frame) {
    ...
    for (y = 0; y < i_height; y++) {
        src[i_width + y * i_stride] = src[i_width - 1 + y * i_stride]; racebench_bug96(0);
    }
    h->mc.memcpy_aligned(src + i_stride * i_height, src + i_stride * (i_height - 1), i_width);
    h->mc.frame_init_lowres_core(src, frame->lowres[0], frame->lowres[1], frame->lowres[2], frame->lowres[3], i_stride, frame->i_stride_lowres, frame->i_width_lowres, frame->i_lines_lowres);
    x264_frame_expand_border_lowres(frame);

    memset(frame->i_cost_est, -1, sizeof(frame->i_cost_est)); racebench_bug15(1);
    ...
    for (y = 0; y <= !!h->param.i_bframe; y++) {
        for (x = 0; x <= h->param.i_bframe; x++) {
            frame->lowres_mvs[y][x][0][0] = 0x7FFF; racebench_bug66(0);
        }
    }
}

static void frame_init_lowres_core(uint8_t *src0, uint8_t *dst0, uint8_t *dsth, uint8_t *dstv, uint8_t *dstc, int src_stride, int dst_stride, int width, int height) {
    int x, y;
    for (y = 0; y < height; y++) {
        uint8_t *src1 = src0 + src_stride;
        uint8_t *src2 = src1 + src_stride; racebench_bug0(0);
        for (x = 0; x < width; x++) {
...
void x264_frame_filter(x264_t *h, x264_frame_t *frame, int mb_y, int b_end) {
    ...
    const int width = frame->i_width[0]; racebench_bug8(0);
    ...
    for (y = 0; y <= b_interlaced; y++, offs += frame->i_stride[0]) {
        h->mc.hpel_filter(frame->filtered[1] + offs, frame->filtered[2] + offs, frame->filtered[3] + offs, frame->plane[0] + offs, stride, width + 16, height - start);
    }
