...
#define OPT2(STR0, STR1) else if (!strcmp(name, STR0) || !strcmp(name, STR1))
    ...
    OPT("asm") {
        p->cpu = isdigit(value[0]) ? atoi(value) : !strcmp(value, "auto") || atobool(value) ? x264_cpu_detect() : 0;
        if (b_error) {
    ...
    OPT("repeat-headers")
    p->b_repeat_headers = atobool(value);
    else return X264_PARAM_BAD_NAME;
#undef OPT
#undef OPT2
#undef atobool
#undef atoi
...
char *x264_param2string(x264_param_t *p, int b_res) {
    ...
    if (p->i_bframe) {
        s += sprintf(s, " b_pyramid=%d b_adapt=%d b_bias=%d direct=%d wpredb=%d", p->b_bframe_pyramid, p->i_bframe_adaptive, p->i_bframe_bias, p->analyse.i_direct_mv_pred, p->analyse.b_weighted_bipred);
    }

    s += sprintf(s, " keyint=%d keyint_min=%d scenecut=%d%s", p->i_keyint_max, p->i_keyint_min, p->i_scenecut_threshold, p->b_pre_scenecut ? "(pre)" : "");

    s += sprintf(s, " rc=%s", p->rc.i_rc_method == X264_RC_ABR ? (p->rc.b_stat_read ? "2pass" : p->rc.i_vbv_buffer_size ? "cbr" : "abr") : p->rc.i_rc_method == X264_RC_CRF ? "crf" : "cqp");
