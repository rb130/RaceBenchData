...
#define PREDICT_4x4_DC(v) SRC32(0, 0) = SRC32(0, 1) = SRC32(0, 2) = SRC32(0, 3) = v;
...
static void predict_4x4_dc(uint8_t *src) {
    uint32_t dc = ((SRC(-1, 0) + SRC(-1, 1) + SRC(-1, 2) + SRC(-1, 3) + SRC(0, -1) + SRC(1, -1) + SRC(2, -1) + SRC(3, -1) + 4) >> 3) * 0x01010101;
    PREDICT_4x4_DC(dc);
}
...
static void predict_4x4_ddl(uint8_t *src) {
    PREDICT_4x4_LOAD_TOP PREDICT_4x4_LOAD_TOP_RIGHT SRC(0, 0) = F2(t0, t1, t2);
...
static void predict_4x4_ddr(uint8_t *src) {
    ...
    PREDICT_4x4_LOAD_LEFT PREDICT_4x4_LOAD_TOP SRC(3, 0) = F2(t3, t2, t1);
...
static void predict_4x4_vr(uint8_t *src) {
    const int lt = SRC(-1, -1);
    PREDICT_4x4_LOAD_LEFT PREDICT_4x4_LOAD_TOP SRC(0, 3) = F2(l2, l1, l0);
    SRC(0, 2) = F2(l1, l0, lt);
    SRC(0, 1) = SRC(1, 3) = F2(l0, lt, t0);
    SRC(0, 0) = SRC(1, 2) = F1(lt, t0);
    SRC(1, 1) = SRC(2, 3) = F2(lt, t0, t1);
    SRC(1, 0) = SRC(2, 2) = F1(t0, t1);
    SRC(2, 1) = SRC(3, 3) = F2(t0, t1, t2); racebench_bug90(1);
...
static void predict_4x4_hd(uint8_t *src) {
    const int lt = SRC(-1, -1);
    PREDICT_4x4_LOAD_LEFT PREDICT_4x4_LOAD_TOP SRC(0, 3) = F1(l2, l3);
    SRC(1, 3) = F2(l1, l2, l3);
...
static void predict_4x4_vl(uint8_t *src) {
    PREDICT_4x4_LOAD_TOP PREDICT_4x4_LOAD_TOP_RIGHT SRC(0, 0) = F1(t0, t1);
    SRC(0, 1) = F2(t0, t1, t2);
...
static void predict_4x4_hu(uint8_t *src) {
    PREDICT_4x4_LOAD_LEFT SRC(0, 0) = F1(l0, l1);
    SRC(1, 0) = F2(l0, l1, l2);
    SRC(2, 0) = SRC(0, 1) = F1(l1, l2);
    SRC(3, 0) = SRC(1, 1) = F2(l1, l2, l3);
    SRC(2, 1) = SRC(0, 2) = F1(l2, l3);
    SRC(3, 1) = SRC(1, 2) = F2(l2, l3, l3);
    SRC(3, 2) = SRC(1, 3) = SRC(0, 3) = SRC(2, 2) = SRC(2, 3) = SRC(3, 3) = l3;
}

#define PL(y) edge[14 - y] = F2(SRC(-1, y - 1), SRC(-1, y), SRC(-1, y + 1));
#define PT(x) edge[16 + x] = F2(SRC(x - 1, -1), SRC(x, -1), SRC(x + 1, -1));
...
void x264_predict_8x8_filter(uint8_t *src, uint8_t edge[33], int i_neighbor, int i_filters) {
    ...
    if (i_filters & MB_LEFT) {
        ...
        PL(1) PL(2) PL(3) PL(4) PL(5) PL(6) edge[7] = (SRC(-1, 6) + 3 * SRC(-1, 7) + 2) >> 2;
    }

    if (i_filters & MB_TOP) {
        int have_tr = i_neighbor & MB_TOPRIGHT;
        edge[16] = ((have_lt ? SRC(-1, -1) : SRC(0, -1)) + 2 * SRC(0, -1) + SRC(1, -1) + 2) >> 2;
        PT(1) PT(2) PT(3) PT(4) PT(5) PT(6) edge[23] = ((have_tr ? SRC(8, -1) : SRC(7, -1)) + 2 * SRC(7, -1) + SRC(6, -1) + 2) >> 2;

        if (i_filters & MB_TOPRIGHT) {
            if (have_tr) {
                PT(8) PT(9) PT(10) PT(11) PT(12) PT(13) PT(14) edge[31] = edge[32] = (SRC(14, -1) + 3 * SRC(15, -1) + 2) >> 2;
            } else {
...
#define PL(y) UNUSED const int l##y = edge[14 - y];
#define PT(x) UNUSED const int t##x = edge[16 + x];
#define PREDICT_8x8_LOAD_TOPLEFT const int lt = edge[15];
#define PREDICT_8x8_LOAD_LEFT PL(0) PL(1) PL(2) PL(3) PL(4) PL(5) PL(6) PL(7)
#define PREDICT_8x8_LOAD_TOP PT(0) PT(1) PT(2) PT(3) PT(4) PT(5) PT(6) PT(7)
#define PREDICT_8x8_LOAD_TOPRIGHT PT(8) PT(9) PT(10) PT(11) PT(12) PT(13) PT(14) PT(15)

#define PREDICT_8x8_DC(v)                                \
    int y;                                               \
    for (y = 0; y < 8; y++) {                            \
        ((uint32_t *)src)[0] = ((uint32_t *)src)[1] = v; \
        src += FDEC_STRIDE;                              \
    }
...
static void predict_8x8_dc_left(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_LEFT const uint32_t dc = ((l0 + l1 + l2 + l3 + l4 + l5 + l6 + l7 + 4) >> 3) * 0x01010101;
    PREDICT_8x8_DC(dc);
}
static void predict_8x8_dc_top(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_TOP const uint32_t dc = ((t0 + t1 + t2 + t3 + t4 + t5 + t6 + t7 + 4) >> 3) * 0x01010101;
    PREDICT_8x8_DC(dc);
}
static void predict_8x8_dc(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_LEFT PREDICT_8x8_LOAD_TOP const uint32_t dc = ((l0 + l1 + l2 + l3 + l4 + l5 + l6 + l7 + t0 + t1 + t2 + t3 + t4 + t5 + t6 + t7 + 8) >> 4) * 0x01010101;
    PREDICT_8x8_DC(dc);
}
static void predict_8x8_h(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_LEFT
#define ROW(y) ((uint32_t *)(src + y * FDEC_STRIDE))[0] = ((uint32_t *)(src + y * FDEC_STRIDE))[1] = 0x01010101U * l##y
        ROW(0);
    ROW(1);
    ROW(2);
    ROW(3);
...
static void predict_8x8_ddl(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_TOP PREDICT_8x8_LOAD_TOPRIGHT SRC(0, 0) = F2(t0, t1, t2);
...
static void predict_8x8_ddr(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_TOP PREDICT_8x8_LOAD_LEFT PREDICT_8x8_LOAD_TOPLEFT SRC(0, 7) = F2(l7, l6, l5);
    ...
    SRC(4, 0) = SRC(5, 1) = SRC(6, 2) = SRC(7, 3) = F2(t2, t3, t4); racebench_bug43(1);
    SRC(5, 0) = SRC(6, 1) = SRC(7, 2) = F2(t3, t4, t5);
    SRC(6, 0) = SRC(7, 1) = F2(t4, t5, t6);
    SRC(7, 0) = F2(t5, t6, t7);
}
static void predict_8x8_vr(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_TOP PREDICT_8x8_LOAD_LEFT PREDICT_8x8_LOAD_TOPLEFT SRC(0, 6) = F2(l5, l4, l3);
    SRC(0, 7) = F2(l6, l5, l4);
...
static void predict_8x8_hd(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_TOP PREDICT_8x8_LOAD_LEFT PREDICT_8x8_LOAD_TOPLEFT int p1 = pack8to16(F1(l6, l7), F2(l5, l6, l7));
    ...
    SRC32(4, 6) = SRC32(0, 4) = pack16to32(p4, p5); racebench_bug54(1);
...
static void predict_8x8_vl(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_TOP PREDICT_8x8_LOAD_TOPRIGHT SRC(0, 0) = F1(t0, t1);
    SRC(0, 1) = F2(t0, t1, t2);
    ...
    SRC(5, 6) = SRC(6, 4) = SRC(7, 2) = F1(t8, t9); racebench_bug38(0);
...
static void predict_8x8_hu(uint8_t *src, uint8_t edge[33]) {
    PREDICT_8x8_LOAD_LEFT int p1 = pack8to16(F1(l0, l1), F2(l0, l1, l2));
    int p2 = pack8to16(F1(l1, l2), F2(l1, l2, l3)); racebench_bug94(1);
    ...
    SRC32(0, 0) = pack16to32(p1, p2); racebench_bug8(1);
    ...
    SRC32(4, 5) = SRC32(4, 6) = SRC32(0, 7) = SRC32(4, 7) = pack16to32(p8, p8); racebench_bug19(1); racebench_bug83(0);
}
