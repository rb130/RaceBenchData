...
static int Encode(x264_param_t *param, cli_opt_t *opt) {
    ...
    for (i_frame = 0, i_file = 0; b_ctrl_c == 0 && (i_frame < i_frame_total || i_frame_total == 0);) {
        ...
        pic.i_pts = (int64_t)i_frame * param->i_fps_den; racebench_bug49(1);

        if (opt->qpfile) {
            parse_qpfile(opt, &pic, i_frame + opt->i_seek);
        } else {

            pic.i_type = X264_TYPE_AUTO; racebench_bug73(1); racebench_bug82(1);
            pic.i_qpplus1 = 0;
        }

        i_file += Encode_frame(h, opt->hout, &pic); racebench_bug13(0); racebench_bug25(1); racebench_bug32(1);

        i_frame++; racebench_bug9(1); racebench_bug26(0); racebench_bug67(0); racebench_bug91(1);
