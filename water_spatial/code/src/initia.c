            ...
            for (j = 0; j < BOX_PER_SIDE; j++)
                for (k = 0; k < BOX_PER_SIDE; k++)
                    for (m = 0; m < Part_per_box; m++) {
                        ...
                        if (X_INDEX >= BOX_PER_SIDE)
                            X_INDEX -= 1;
                        if (Y_INDEX >= BOX_PER_SIDE)
                            Y_INDEX -= 1;
                        if (Z_INDEX >= BOX_PER_SIDE)
                            Z_INDEX -= 1;
    ...
    for (i = 0; i < BOX_PER_SIDE; i++) {
        for (j = 0; j < BOX_PER_SIDE; j++) {
            for (k = 0; k < BOX_PER_SIDE; k++) {
                ...
                while (curr_ptr) {
                    SUX = SUX + (pow((curr_ptr->mol.F[VEL][XDIR][H1] - SUMX), 2.0) + pow((curr_ptr->mol.F[VEL][XDIR][H2] - SUMX), 2.0)) / HMAS + pow((curr_ptr->mol.F[VEL][XDIR][O] - SUMX), 2.0) / OMAS;

                    SUY = SUY + (pow((curr_ptr->mol.F[VEL][YDIR][H1] - SUMY), 2.0) + pow((curr_ptr->mol.F[VEL][YDIR][H2] - SUMY), 2.0)) / HMAS + pow((curr_ptr->mol.F[VEL][YDIR][O] - SUMY), 2.0) / OMAS;

                    SUZ = SUZ + (pow((curr_ptr->mol.F[VEL][ZDIR][H1] - SUMZ), 2.0) + pow((curr_ptr->mol.F[VEL][ZDIR][H2] - SUMZ), 2.0)) / HMAS + pow((curr_ptr->mol.F[VEL][ZDIR][O] - SUMZ), 2.0) / OMAS;
    ...
    for (i = 0; i < BOX_PER_SIDE; i++) {
        for (j = 0; j < BOX_PER_SIDE; j++) {
            for (k = 0; k < BOX_PER_SIDE; k++) {
                ...
                while (curr_ptr) {
                    for (atom = 0; atom < NATOMS; atom++) {
                        curr_ptr->mol.F[VEL][XDIR][atom] = (curr_ptr->mol.F[VEL][XDIR][atom] - SUMX) * SUX / XMAS[atom];
                        curr_ptr->mol.F[VEL][YDIR][atom] = (curr_ptr->mol.F[VEL][YDIR][atom] - SUMY) * SUY / XMAS[atom];
                        curr_ptr->mol.F[VEL][ZDIR][atom] = (curr_ptr->mol.F[VEL][ZDIR][atom] - SUMZ) * SUZ / XMAS[atom];
                    }
                    curr_ptr = curr_ptr->next_mol;
                }
            }
...
double xrand(xl, xh) double xl, xh;
{
    long random();
    double x;
