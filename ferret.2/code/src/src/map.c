...
static int cass_map_dump_private(cass_map_t *map) {
    ...
    if (cass_write_size(&map->vtable.len, 1, fout) != 1) {
        return CASS_ERR_IO;
    }

    ARRAY_BEGIN_FOREACH(map->vtable, const char *p) {
        if (cass_write_pchar(p, fout) != 0) {
            return CASS_ERR_IO;
        }
    }
    ARRAY_END_FOREACH;
...
static int cass_map_load_private(cass_map_t *map) {
    ...
    if (cass_read_size(&tb_size, 1, fin) != 1) {
        return CASS_ERR_IO;
    }
...
int cass_map_disassociate_table(cass_map_t *map, int32_t table_id) {
    ...
    for (ix = 0; ix < map->table_ids.len; ix++) {
        if (map->table_ids.data[ix] == table_id) {
            break;
        }
    }
...
int cass_map_insert(cass_map_t *map, cass_vecset_id_t *id, char *dataobj_name) {
    char *name;
...
int cass_map_release(cass_map_t *map) {
    assert(!map->dirty);
...
int cass_map_load(cass_map_t *map) {
...
int cass_map_checkpoint(cass_map_t *map, CASS_FILE *out) {
    assert(map->magic == MAP_MAGIC_SIG);
    if (cass_write_int32(&map->magic, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_pchar(map->name, out) != 0) {
        return CASS_ERR_IO;
    }
    if (cass_write_uint32(&map->flag, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_size(&map->table_ids.len, 1, out) != 1) {
        return CASS_ERR_IO;
    }
    if (cass_write_int32(map->table_ids.data, map->table_ids.len, out) != map->table_ids.len) {
        return CASS_ERR_IO;
    }
...
int cass_map_restore(cass_map_t **_map, cass_env_t *env, CASS_FILE *in) {
    ...
    map->env = env;
    map->refcnt++;
    if (cass_read_int32(&map->magic, 1, in) != 1) {
        goto err;
    }
    assert(map->magic == MAP_MAGIC_SIG);
    ...
    if (cass_read_uint32(&map->flag, 1, in) != 1) {
        goto err;
    }
    ...
    if (n != 0 && map->table_ids.data == NULL) {
        goto err;
    }

    ret = CASS_ERR_IO;
    if (n > 0) {
        if (cass_read_int32((int32_t *)map->table_ids.data, n, in) != n) {
            goto err;
        }
        map->table_ids.len = n;
    }
...
int cass_map_drop(cass_map_t *map) {

    return -1;
}

int cass_map_free(cass_map_t *map) {
    assert(map != NULL);
    map->refcnt--;
    if (map->refcnt > 0) {
        return 0;
