...
static int im_cooc_sym(im, m, xpos, ypos, xsize, ysize, dx, dy) IMAGE *im, *m;
...
static int im_cooc_ord(im, m, xpos, ypos, xsize, ysize, dx, dy) IMAGE *im, *m;
int xpos, ypos, xsize, ysize;
int dx, dy;
{
    PEL *input, *cpinput;
...
int im_cooc_matrix(IMAGE *im, IMAGE *m, int xp, int yp, int xs, int ys, int dx, int dy, int flag) {
    if (flag == 0) {
...
int im_cooc_asm(IMAGE *m, double *asmoment) {
    ...
    if (m->Xsize != 256 || m->Ysize != 256 || m->Bands != 1 || m->BandFmt != IM_BANDFMT_DOUBLE) {
...
static void stats(buffer, size, pmean, pstd) double *buffer;
int size;
...
int im_cooc_correlation(IMAGE *m, double *correlation) {
    double mcol, stdcol, mrow, stdrow;
    double *pbuf;
    double *cpbuf;
    double dtemp;
    register int i, j;
