...
static int erode_stop(void *vseq, void *a, void *b) {
...
static void *erode_start(IMAGE *out, void *a, void *b) {
    IMAGE *in = (IMAGE *)a;
...
static int erode_gen(REGION * or, void *vseq, void *a, void *b) {
    SeqInfo *seq = (SeqInfo *)vseq;
    INTMASK *msk = (INTMASK *)b;
    REGION *ir = seq->ir;
...
int im_erode(IMAGE *in, IMAGE *out, INTMASK *m) {
    ...
    if (!t1 || im_embed(in, t1, 1, m->xsize / 2, m->ysize / 2, in->Xsize + m->xsize - 1, in->Ysize + m->ysize - 1) || im_erode_raw(t1, out, m)) {
        return (-1);
    }
