...
static void decode_print_flags(const Decode *decode, int ndecode, int flags) {
    int i;
    ...
    for (i = 0; i < ndecode; i++)
        if (decode[i].value & flags) {
            printf("[");
            decode_print(decode, ndecode, decode[i].value & flags);
            printf("] ");
...
static int lgrab_ioctl(LGrab *lg, int request, void *argp) {
    if (!lg->fd) {
    ...
    if (ioctl(lg->fd, request, argp) < 0) {
        im_error("lgrab_ioctl", _("ioctl(0x%x) failed: %s"), (unsigned int)request, strerror(errno));
        return (-1);
    }
...
static void lgrab_destroy(LGrab *lg) {
    if (lg->fd != -1) {
        int zero = 0;
...
static LGrab *lgrab_new(const char *device) {
    ...
    if (!lg->device || (lg->fd = open(lg->device, O_RDWR)) == -1) {
        im_error("lgrab_new", _("cannot open video device \"%s\""), lg->device);
        lgrab_destroy(lg);
        return (NULL);
    }
    ...
    if (!(lg->capability.type & VID_TYPE_CAPTURE)) {
        im_error("lgrab_new", "%s", _("card cannot capture to memory"));
        lgrab_destroy(lg);
        return (NULL);
    }

    for (i = 0; i < IM_MIN(lg->capability.channels, IM_MAXCHANNELS); i++) {
    ...
    if (lgrab_ioctl(lg, VIDIOCGWIN, &lg->window) || lgrab_ioctl(lg, VIDIOCGPICT, &lg->picture)) {
        lgrab_destroy(lg);
        return (NULL);
    }
...
#ifdef DEBUG
static void lgrab_dump_capability(struct video_capability *capability) {
    ...
    decode_print_flags(decode_vtype, IM_NUMBER(decode_vtype), capability->type);
    printf("\n");
}

static void lgrab_dump_channel(struct video_channel *channel) {
    printf("channel->channel = %d\n", channel->channel);
    printf("channel->name = \"%s\"\n", channel->name);
    printf("channel->tuners = %d\n", channel->tuners);

    printf("channel->flags = ");
    decode_print_flags(decode_ctype, IM_NUMBER(decode_ctype), channel->flags);
    printf("\n");
    printf("channel->type = ");
    decode_print(decode_type, IM_NUMBER(decode_type), channel->type);
    printf("\n");
    printf("channel->norm = %d\n", channel->norm);
}

static void lgrab_dump_picture(struct video_picture *picture) {
    ...
    decode_print(decode_palette, IM_NUMBER(decode_palette), picture->palette);
    printf("\n");
}

static void lgrab_dump(LGrab *lg) {
    int i;
...
static int lgrab_set_capture_size(LGrab *lg, int width, int height) {
    lg->c_width = width;
    lg->c_height = height;
    ...
    if (!(lg->capture_buffer = mmap(0, lg->capture_size, PROT_READ | PROT_WRITE, MAP_SHARED, lg->fd, 0))) {
        im_error("lgrab_set_capture_size", "%s", _("unable to map memory"));
        return (-1);
    }
...
static int lgrab_set_channel(LGrab *lg, int channel) {
    if (channel < 0 || channel >= lg->capability.channels) {
        im_error("lgrab_set_channel", _("channel not between 0 and %d"), lg->capability.channels - 1);
        return (-1);
    }
...
static int lgrab_set_brightness(LGrab *lg, int brightness) {
...
static int lgrab_set_colour(LGrab *lg, int colour) {
...
static int lgrab_set_contrast(LGrab *lg, int contrast) {
...
static int lgrab_set_hue(LGrab *lg, int hue) {
...
static int lgrab_set_ngrabs(LGrab *lg, int ngrabs) {
    lg->c_ngrabs = IM_CLIP(1, ngrabs, 1000);

    return (0);
}

static int lgrab_capture1(LGrab *lg) {
    lg->mmap.format = lg->picture.palette;
    lg->mmap.frame = 0;
    lg->mmap.width = lg->c_width;
    lg->mmap.height = lg->c_height;
    if (lgrab_ioctl(lg, VIDIOCMCAPTURE, &lg->mmap) || lgrab_ioctl(lg, VIDIOCSYNC, &lg->mmap.frame))
        return (-1);

    return (0);
}

static int lgrab_capturen(LGrab *lg) {
    if (lg->c_ngrabs == 1) {
...
static int lgrab_capture(LGrab *lg, IMAGE *im) {
    int x, y;
    unsigned char *line;
    ...
    if (im_outcheck(im))
        return (-1);
    im_initdesc(im, lg->c_width, lg->c_height, 3, IM_BBITS_BYTE, IM_BANDFMT_UCHAR, IM_CODING_NONE, IM_TYPE_MULTIBAND, 1.0, 1.0, 0, 0);
    if (im_setupout(im))
        return (-1);
    if (!(line = IM_ARRAY(im, IM_IMAGE_SIZEOF_LINE(im), unsigned char)))
        return (-1);

    for (y = 0; y < lg->c_height; y++) {
        unsigned char *p = (unsigned char *)lg->capture_buffer + y * IM_IMAGE_SIZEOF_LINE(im);
...
int im_video_v4l1(IMAGE *im, const char *device, int channel, int brightness, int colour, int contrast, int hue, int ngrabs) {
    ...
    if (lgrab_set_capture_size(lg, lg->capability.maxwidth, lg->capability.maxheight) || lgrab_set_channel(lg, channel) || lgrab_set_brightness(lg, brightness) || lgrab_set_colour(lg, colour) || lgrab_set_contrast(lg, contrast) || lgrab_set_hue(lg, hue) || lgrab_set_ngrabs(lg, ngrabs) || lgrab_capture(lg, im)) {
        lgrab_destroy(lg);
        return (-1);
    }
...
int im_video_v4l1(IMAGE *im, const char *device, int channel, int brightness, int colour, int contrast, int hue, int ngrabs) {
    im_error("im_video_v4l1", "%s", _("compiled without im_video_v4l1 support"));
    return (-1);
}
