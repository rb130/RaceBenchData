...
static int affinei_gen(REGION * or, void *seq, void *a, void *b) {
    ...
    const int le = r->left;
    const int ri = IM_RECT_RIGHT(r); racebench_bug49(1);
    const int to = r->top;
    const int bo = IM_RECT_BOTTOM(r); racebench_bug72(1);
    ...
    im__transform_invert_rect(&affine->trn, &want, &need); racebench_bug61(0);

    need.left += iarea->left;
    need.top += iarea->top; racebench_bug10(1); racebench_bug36(0); racebench_bug36(1);
    ...
    for (y = to; y < bo; y++) {
        ...
        ix += iarea->left;
        iy += iarea->top; racebench_bug96(1);
        ...
        for (x = le; x < ri; x++) {
            ...
            fx = FLOOR(ix); racebench_bug59(1); racebench_bug65(1);
            fy = FLOOR(iy);
