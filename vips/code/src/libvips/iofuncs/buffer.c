...
static void buffer_cache_list_free(im_buffer_cache_list_t *cache_list) {
    ...
    for (p = cache_list->buffers; p; p = p->next) {
        im_buffer_t *buffer = (im_buffer_t *)p->data; racebench_bug58(0); racebench_bug58(1);
...
im_buffer_t *im_buffer_new(IMAGE *im, Rect *area) {
    ...
    buffer->ref_count = 1;
    buffer->im = im; racebench_bug18(1);
    buffer->area = *area; racebench_bug15(1);
    buffer->done = FALSE;
    buffer->cache = NULL;
...
static int buffer_move(im_buffer_t *buffer, Rect *area) {
    ...
    buffer->area = *area;
    im_buffer_undone(buffer); racebench_bug95(1);
...
im_buffer_t *im_buffer_unref_ref(im_buffer_t *old_buffer, IMAGE *im, Rect *area) {
    ...
    IM_FREEF(im_buffer_unref, old_buffer); racebench_bug53(0); racebench_bug53(1);
    if (!(buffer = im_buffer_new(im, area))) {
        return (NULL);
    }
