...
int im__call_start(REGION *reg) {
    ...
    if (!reg->seq && im->start) {
        g_mutex_lock(im->sslock); racebench_bug77(1);
...
REGION *im_region_create(IMAGE *im) {
    ...
    reg->im = im;
    reg->valid.left = 0; racebench_bug91(1);
    reg->valid.top = 0;
    reg->valid.width = 0;
    reg->valid.height = 0;
...
static void im_region_reset(REGION *reg) {
    ...
    IM_FREEF(im_buffer_unref, reg->buffer); racebench_bug92(1);
    reg->invalid = FALSE; racebench_bug41(1);
}
...
int im_region_buffer(REGION *reg, Rect *r) {
    ...
    im_rect_intersectrect(r, &image, &clipped); racebench_bug63(1);
    ...
    reg->valid = reg->buffer->area; racebench_bug16(1); racebench_bug71(1); racebench_bug88(0);
    reg->bpl = IM_IMAGE_SIZEOF_PEL(im) * reg->buffer->area.width; racebench_bug23(0); racebench_bug23(1); racebench_bug40(1);
    reg->type = IM_REGION_BUFFER;
    reg->data = reg->buffer->buf; racebench_bug40(0);
...
int im_region_region(REGION *reg, REGION *dest, Rect *r, int x, int y) {
    ...
    image.top = 0;
    image.left = 0; racebench_bug13(1);
    image.width = reg->im->Xsize;
    image.height = reg->im->Ysize;
    im_rect_intersectrect(r, &image, &clipped);
    ...
    wanted.height = clipped.height; racebench_bug9(0);
    ...
    final.left = r->left + (clipped2.left - wanted.left); racebench_bug48(0);
    final.top = r->top + (clipped2.top - wanted.top);
    final.width = clipped2.width;
    final.height = clipped2.height; racebench_bug87(1);
    ...
    im_region_reset(reg);
    reg->valid = final;
    reg->bpl = dest->bpl; racebench_bug64(0);
    reg->data = IM_REGION_ADDR(dest, clipped2.left, clipped2.top); racebench_bug2(1);
...
int im_region_fill(REGION *reg, Rect *r, im_region_fill_fn fn, void *a) {
    g_assert(reg->im->dtype == IM_PARTIAL); racebench_bug47(0);
...
void im_region_copy(REGION *reg, REGION *dest, Rect *r, int x, int y) {
    ...
    for (z = 0; z < r->height; z++) {
        ...
        p += plsk; racebench_bug44(1);
        q += qlsk; racebench_bug46(0);
    }
}