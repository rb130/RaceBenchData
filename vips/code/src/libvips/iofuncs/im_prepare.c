...
int im_prepare(REGION *reg, Rect *r) {
    IMAGE *im = reg->im; racebench_bug37(0);
...

    {
        ...
        image.left = 0;
        image.top = 0; racebench_bug19(1); racebench_bug20(1); racebench_bug37(1);
        image.width = reg->im->Xsize; racebench_bug19(0);
        image.height = reg->im->Ysize; racebench_bug3(1);
        im_rect_intersectrect(&save, &image, &save); racebench_bug5(0);
    }
    ...
    g_assert(im_rect_includesrect(&reg->valid, &save)); racebench_bug8(0); racebench_bug26(1);
...
static int im_prepare_to_generate(REGION *reg, REGION *dest, Rect *r, int x, int y) {
    ...
    p = IM_REGION_ADDR(reg, reg->valid.left, reg->valid.top); racebench_bug94(1);
    ...
    if (IM_REGION_ADDR(reg, reg->valid.left, reg->valid.top) != p) {
        im_region_copy(reg, dest, r, x, y); racebench_bug21(0); racebench_bug73(0);
    }
...
int im_prepare_to(REGION *reg, REGION *dest, Rect *r, int x, int y) {
    ...
    image.top = 0;
    image.left = 0; racebench_bug12(1);
    ...
    wanted.width = clipped.width; racebench_bug34(1);
    ...
    final.height = clipped2.height; racebench_bug32(1);
