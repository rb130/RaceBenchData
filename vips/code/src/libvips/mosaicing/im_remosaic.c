...
static IMAGE *remosaic(JoinNode *node, RemosaicData *rd) {
    ...
    if (!im) {
        im_error("im_remosaic", _("file \"%s\" not found"), node->name);
        return (NULL);
    }
    ...
    if ((p = im_strrstr(filename, rd->old_str))) {
        ...
        im_strncpy(p + rd->new_len, im->filename + offset + rd->old_len, FILENAME_MAX - offset - rd->new_len);
    }
...
int im_remosaic(IMAGE *in, IMAGE *out, const char *old_str, const char *new_str) {
    ...
    if (!(st = im__build_symtab(out, SYM_TAB_SIZE)) || im__parse_desc(st, in)) {
        return (-1);
    }
