...
#define FUNCTION_NAME "im_gradcor_raw"
    ...
    if (im_check_uncoded("im_gradcor", large) || im_check_mono("im_gradcor", large) || im_check_uncoded("im_gradcor", small) || im_check_mono("im_gradcor", small) || im_check_format_same("im_gradcor", large, small) || im_check_int("im_gradcor", large)) {
        return (-1);
    }
...
#define FUNCTION_NAME "im_gradcor"
    ...
    if (!t1 || im_embed(in, t1, 1, ref->Xsize / 2, ref->Ysize / 2, in->Xsize + ref->Xsize - 1, in->Ysize + ref->Ysize - 1) || im_gradcor_raw(t1, ref, out)) {
        return (-1);
    }
...
#define FUNCTION_NAME "im_grad_x"
    ...
    if (im_check_uncoded("im_grad_x", in) || im_check_mono("im_grad_x", in) || im_check_int("im_grad_x", in)) {
        return (-1);
    }
...
#define FUNCTION_NAME "im_grad_y"
    ...
    if (im_check_uncoded("im_grad_y", in) || im_check_mono("im_grad_y", in) || im_check_int("im_grad_y", in)) {
        return (-1);
    }
...
static int gradcor_gen(REGION *to_make, void *vptr_seq, void *unrequired, void *vptr_grads) {
    ...
    Rect require = {to_make->valid.left, to_make->valid.top, to_make->valid.width + small_xgrad->Xsize, to_make->valid.height + small_ygrad->Ysize};
...
#define XGRAD_GEN_DEFINITION(TYPE)                                                                                 \
    static int xgrad_gen_##TYPE(REGION *to_make, void *vptr_make_from, void *unrequired, void *unreq2) {           \
        ...
        Rect require = {to_make->valid.left, to_make->valid.top, to_make->valid.width + 1, to_make->valid.height}; \
        if (im_prepare(make_from, &require))                                                                       \
            return -1;                                                                                             \
                                                                                                                   \
        {                                                                                                          \
...
#define YGRAD_GEN_DEFINITION(TYPE)                                                                                 \
    static int ygrad_gen_##TYPE(REGION *to_make, void *vptr_make_from, void *unrequired, void *unreq2) {           \
                                                                                                                   \
        REGION *make_from = (REGION *)vptr_make_from;                                                              \
        Rect require = {to_make->valid.left, to_make->valid.top, to_make->valid.width, to_make->valid.height + 1}; \
        if (im_prepare(make_from, &require))                                                                       \
            return -1;                                                                                             \
                                                                                                                   \
        {                                                                                                          \
