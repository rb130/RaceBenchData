...
static int cont_surf_gen(REGION *to_make, void *seq, void *a, void *b);

static unsigned int calc_cont(REGION *reg, int win_size_less_one, int x_left, int y_top);

int im_contrast_surface(IMAGE *in, IMAGE *out, int half_win_size, int spacing) {
    IMAGE *t1 = im_open_local(out, "im_contrast_surface intermediate", "p");

    if (!t1 || im_embed(in, t1, 1, half_win_size, half_win_size, in->Xsize + DOUBLE(half_win_size), in->Ysize + DOUBLE(half_win_size)) || im_contrast_surface_raw(t1, out, half_win_size, spacing)) {
...
#define FUNCTION_NAME "im_contrast_surface_raw"
    ...
    if (im_piocheck(in, out) || im_check_uncoded(FUNCTION_NAME, in) || im_check_mono(FUNCTION_NAME, in) || im_check_format(FUNCTION_NAME, in, IM_BANDFMT_UCHAR)) {
        return -1;
    }
    ...
    if (DOUBLE(half_win_size) >= LESSER(in->Xsize, in->Ysize)) {
        im_error(FUNCTION_NAME, "%s", _("parameters would result in zero size output image"));
        return -1;
    }
    ...
    return im_generate(out, im_start_one, cont_surf_gen, im_stop_one, in, params);

#undef FUNCTION_NAME
}

static int cont_surf_gen(REGION *to_make, void *seq, void *unrequired, void *b) {

    REGION *make_from = (REGION *)seq;
    cont_surf_params_t *params = (cont_surf_params_t *)b;

    unsigned int *row = (unsigned int *)IM_REGION_ADDR(to_make, to_make->valid.left, to_make->valid.top);
    int xoff;
    int y;
    int bottom = to_make->valid.top + to_make->valid.height;
    size_t lskip = IM_REGION_LSKIP(to_make) / sizeof(unsigned int);

    Rect area = {params->spacing * to_make->valid.left, params->spacing * to_make->valid.top, DOUBLE_ADD_ONE(params->half_win_size) + (params->spacing * (to_make->valid.width - 1)), DOUBLE_ADD_ONE(params->half_win_size) + (params->spacing * (to_make->valid.height - 1))};

    if (im_prepare(make_from, &area) || !im_rect_equalsrect(&make_from->valid, &area)) {
        return -1;
    }
    ...
    for (y = to_make->valid.top; y < bottom; ++y, row += lskip) {
        ...
        for (xoff = 0; xoff < to_make->valid.width; ++xoff) {
            ...
            row[xoff] = calc_cont(make_from, DOUBLE(params->half_win_size), (xoff + to_make->valid.left) * params->spacing, y * params->spacing);
        }
    }
...
static unsigned int calc_cont(REGION *reg, int win_size_less_one, int x_left, int y_top) {
    ...
    for (yoff = 0; yoff < win_size_less_one; ++yoff, row += lskip) {
        for (xoff = 0; xoff < win_size_less_one; ++xoff) {
            contrast += abs(row[xoff + 1] - row[xoff]) + abs(row[xoff + lskip] - row[xoff]);
        }
