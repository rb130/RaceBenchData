...
#define LINREG_START_DEFN(TYPE)                                      \
    static void *linreg_start_##TYPE(IMAGE *out, void *a, void *b) { \
        IMAGE **ins = (IMAGE **)a;                                   \
...
#define LINREG_GEN_DEFN(TYPE)                                                                                    \
    static int linreg_gen_##TYPE(REGION *to_make, void *vseq, void *unrequired, void *b) {                       \
...
#define LINREG_STOP_DEFN(TYPE)                                    \
    static int linreg_stop_##TYPE(void *vseq, void *a, void *b) { \
        linreg_seq_##TYPE *seq = (linreg_seq_##TYPE *)vseq;       \
        if (seq->regs)                                            \
...
#define INCR_ALL_DEFN(TYPE)                                    \
    static void incr_all_##TYPE(TYPE **ptrs, unsigned int n) { \
        TYPE **stop = ptrs + n;                                \
        for (; ptrs < stop; ++ptrs)                            \
            ++*ptrs;                                           \
    }
...
#define SKIP_ALL_DEFN(TYPE)                                                   \
    static void skip_all_##TYPE(TYPE **ptrs, size_t *skips, unsigned int n) { \
        TYPE **stop = ptrs + n;                                               \
