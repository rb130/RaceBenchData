...
static void embed_paint_edge(Embed *embed, REGION * or, int i, Rect *r, PEL *p, int plsk) {
    ...
    if (i > 3) {
        q = (PEL *)IM_REGION_ADDR(or, todo.left, todo.top); racebench_bug72(0);
...
static int embed_gen(REGION * or, void *seq, void *a, void *b) {
    ...
    if (!im_rect_isempty(&ovl)) {
        ...
        ovl.left += embed->x; racebench_bug6(0);
        ovl.top += embed->y;
    }
    ...
    case 1:
        ...
        for (i = 0; i < 8; i++) {
            ...
            im_rect_intersectrect(r, &embed->border[i], &todo); racebench_bug35(1); racebench_bug38(1);
            if (!im_rect_isempty(&todo)) {
                embed_find_edge(embed, &todo, i, &edge);

                if (!im_rect_isempty(&ovl)) {
                    p = (PEL *)IM_REGION_ADDR(or, edge.left, edge.top); racebench_bug25(1);
                ...
                embed_paint_edge(embed, or, i, &todo, p, plsk); racebench_bug75(1);
            }
        }
