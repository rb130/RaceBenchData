...
static int im_phmerge(Rect *larea, Rect *rarea, Rect *outarea) {
...
static int im_pvmerge(Rect *tarea, Rect *barea, Rect *outarea) {
...
static int merge_analysis(int width, int height, IMAGE **in, int xoff, int yoff, int *vxdisp, int *vydisp, int *hxdisp, int *hydisp, Rect *hrect, Rect *vrect) {
    int i, j;
    int curr_im, offset;
    int curr_x, curr_y;
...
static char *find_root(name) char *name;
{
...
static int find_x(name) char *name;
{
    int n;
    char *p;
    char *out = strdup(name);
...
static int find_y(name) char *name;
{
    int m;
    char *p;
    char *out = strdup(name);
...
static int join_leftright(IMAGE *left, IMAGE *right, IMAGE *out, int dx, int dy) {
...
static int join_updown(IMAGE *top, IMAGE *bottom, IMAGE *out, int dx, int dy) {
...
static int merge_up(int width, int height, IMAGE **inp, IMAGE *outp, int xoff, int yoff, int *hxdisp, int *hydisp, Rect *vrect) {
    int dx, dy, first_row;
    ...
    for (i = 0; i <= height; i++) {
        for (j = 0; j <= width; j++) {
            ...
            if (j != 0) {
                ...
                if ((height == 0) && (j == width)) {
                    join_leftright(p_img[partial_no - 3], p_img[partial_no - 2], outp, dx, dy);
                } else {
                    join_leftright(p_img[partial_no - 3], p_img[partial_no - 2], p_img[partial_no - 1], dx, dy);
                }
                ++h_no;
            }
            ++in_no;
        ...
        if ((i > 0) || (height == 0)) {
            if (i < height) {
                ...
                ++v_no;
                join_updown(p_img[first_row], p_img[partial_no - 2], p_img[partial_no - 1], dx, dy);
                first_row = partial_no - 1;
            } else {
