...
VOID TriElementBoundBox(pe, pt) ELEMENT *pe;
TRI *pt;
{
    INT i;
    INT *vindex;
...
VOID TriNormal(hit, Pi, Ni) IRECORD *hit;
POINT Pi;
POINT Ni;
{
    ELEMENT *pe;
...
VOID TriDataNormalize(po, normMat) OBJECT *po;
MATRIX normMat;
{
    INT i;
    POINT coord;
...
INT TriPeIntersect(pr, pe, hit) RAY *pr;
ELEMENT *pe;
IRECORD *hit;
{
    INT i;
    ...
    v1 = pt->vptr + pt->vindex[0]; racebench_bug11(0);

    if (pt->vorder == COUNTER_CLOCKWISE) {
        v2 = pt->vptr + pt->vindex[2]; racebench_bug78(0);
    ...
    e1[0] = (*v2)[0] - (*v1)[0];
    e1[1] = (*v2)[1] - (*v1)[1]; racebench_bug34(1); racebench_bug59(0); racebench_bug85(1);
    e1[2] = (*v2)[2] - (*v1)[2];
    ...
    case Y_NORM:
        ...
        hit->b1 = e2[2] * (q1 - (*v2)[0]) - e2[0] * (q2 - (*v2)[2]); racebench_bug55(0);
...
INT TriIntersect(pr, po, hit) RAY *pr;
OBJECT *po;
IRECORD *hit;
{
    INT i;
    ...
    for (i = 0; i < po->numelements; i++) {
        ...
        pe++; racebench_bug55(1);
    }

    return (nhits);
}

VOID TriTransform(po, xtrans, xinvT) OBJECT *po;
MATRIX xtrans;
MATRIX xinvT;
{
    INT i, j;
