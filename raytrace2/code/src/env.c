...
BOOL VerifyColorRange(c) COLOR c;
{
    if (c[0] < 0.0 || c[0] > 1.0 || c[1] < 0.0 || c[1] > 1.0 || c[2] < 0.0 || c[2] > 1.0) {
...
VOID TransformLights(m) MATRIX m;
{
    INT i;
    LIGHT *lp;
...
VOID ViewRotate(M, x, y, z) MATRIX M;
REAL x, y, z;
{
    REAL r, rx;
...
VOID TransformViewRay(tray) POINT tray;
{ VecMatMult(tray, View.vtransInv, tray); }

VOID NormalizeEnv(normMat) MATRIX normMat;
{
    POINT tmp;
...
VOID ReadEnvFile(EnvFileName) CHAR *EnvFileName;
{
    INT i, j;
    INT stat;
    INT dummy;
    ...
    while (fscanf(pf, "%29s", command) != EOF) {
        ...
        case OP_LIGHT:
            ...
            stat = fscanf(pf, "%lf %lf %lf %lf %lf %lf", &(lptr->pos[0]), &(lptr->pos[1]), &(lptr->pos[2]), &(lptr->col[0]), &(lptr->col[1]), &(lptr->col[2]));
        ...
        case OP_GEOM_FILE:
            stat = fscanf(pf, " %79s", GeoFileName);
            if (stat != 1) {
        ...
        case OP_RL_FILE:
            stat = fscanf(pf, " %79s", PicFileName);
            if (stat != 1) {
