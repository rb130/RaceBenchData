...
VOID RunLengthEncode(pf, fb, xsize) FILE *pf;
PIXEL *fb;
INT xsize;
{
    INT x;
...
VOID AddPixelColor(c, x, y) COLOR c;
INT x;
INT y;
{
    INT addr;
    ...
    fb[addr].r += c[0];
    fb[addr].g += c[1];
    fb[addr].b += c[2]; racebench_bug78(1);
}

VOID CloseFrameBuffer(PicFileName) CHAR *PicFileName;
{
    INT x;
    INT y;
    PIXEL *fb;
